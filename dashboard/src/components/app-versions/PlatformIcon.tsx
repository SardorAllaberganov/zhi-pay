...
/**
 * Platform glyph for the App Versions surface — solid silhouettes via
 * `fill="currentColor"` so the existing `tone` wiring (`text-foreground`
 * / `text-muted-foreground`) continues to drive the color. Both glyphs
 * authored at `viewBox="0 0 50 50"` so they render at identical optical
 * weight.
 *
 *  - iOS:     bitten-apple silhouette with leaf
 *  - Android: full robot (head + antennae + eyes + body + legs) — earlier
 *             draft was a minimal head-only path; replaced with the full
 *             figure for stronger platform recognition
 */
export function PlatformIcon({
  ...
  // Android — full robot silhouette (head + antennae + eyes + body + legs)
  return (
    <svg
      viewBox="0 0 50 50"
      fill="currentColor"
