...
export function OwnerCard({ owner }: Props) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t('admin.recipients.detail.owner')}</CardTitle>
      </CardHeader>
      <CardContent>
        {!owner ? (
          <div className="text-sm text-muted-foreground italic">
            {t('admin.recipients.detail.owner.unknown')}
          </div>
        ) : (
          <div className="flex flex-col gap-4">
                ...
                {owner.pinfl ? (
                  <div className="text-sm text-muted-foreground">
                    <span className="text-xs uppercase tracking-wider mr-1">
                      {t('admin.recipients.detail.owner.pinfl')}
                    </span>
                    <span className="tabular font-mono">{maskPinfl(owner.pinfl)}</span>
                  </div>
                ) : (
                  <div className="text-sm text-muted-foreground italic">
                    {t('admin.recipients.detail.owner.pinfl-not-verified')}
                  </div>
                )}
            ...
            <div className="pt-1">
              ...
              >
                {t('admin.recipients.detail.owner.open-profile')}
