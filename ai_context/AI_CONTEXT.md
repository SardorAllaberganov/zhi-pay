...
**Seven admin surfaces live** (Overview / Transfers / Transfer Detail / KYC Queue / AML Triage / Users / **Cards**). Dashboard prototype under `dashboard/` (Vite + React 18 + TS + Tailwind + shadcn).
...
Next phases: the rest of the 12 placeholder routes — driven by user prompts. Mobile app and full brand work still pending. Build-progress snapshot lives in [`docs/product_states.md`](../docs/product_states.md).
...
│       ├── data/mockUsers.ts          # 50-user deterministic mock + Sardor demo profile + per-user devices/recipients/limit-usage/kyc-history + admin mutators (block/unblock/soft-delete/untrust/hard-delete-recipient/reset-devices/reverify-kyc/blacklist-phone/generate-report) + cross-store sync to AML's blockAmlUser AND to mockCards.freezeAllUserActiveCards on block. Card data + freeze/unfreeze are re-exports from mockCards (single source of truth)
...
│       └── pages/{Overview,Transfers,TransferDetail,KycQueue,AmlTriage,AmlTriageNew,Users,UserDetail,Cards,CardDetail,Placeholder}.tsx + router.tsx
...
- ☐ Other 12 admin sub-pages — placeholder routes, content TBD (`/recipients`, `/blacklist`, `/kyc-tiers`, `/audit-log`, `/fx-config`, `/commission-rules`, `/services`, `/app-versions`, `/error-codes`, `/stories`, `/news`, `/notifications`)
