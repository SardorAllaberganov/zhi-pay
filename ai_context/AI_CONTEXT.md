...
**Transfers Monitor + Transfer Detail (forensic) + KYC Review Queue (master-detail) live.** Dashboard prototype under `dashboard/` (Vite + React 18 + TS + Tailwind + shadcn). Four admin surfaces functional:
...
| Routes (admin) | **partially nested** — Transfers + KYC Queue use nested `/operations/*` paths. Other admin pages stay flat (`/services`, `/fx-config`, `/aml-triage`, etc.) until each phase migrates them. Sidebar entries for Transfers + KYC Queue point to the nested paths; `/transfers` and `/kyc-queue` redirect to their nested forms for back-compat |
...
│       │   ├── KycDetailPane.tsx      # top bar + scrollable body + sticky-bottom ActionBar
...
│       └── pages/{Overview,Transfers,TransferDetail,KycQueue,Placeholder}.tsx + router.tsx
...
- ☐ Other 12 admin sub-pages — placeholder routes, content TBD
