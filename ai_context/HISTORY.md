...
- **Files modified**:
  - Figma file `p8pxXYApMNCpzQn0XkoIw9` — 3 new COMPONENT nodes on `❖ Components` page (`0:1`).
  - `docs/product_states.md` — Last-updated stamp updated to point at #7; Mobile-screens row appended with full follow-up #7 detail.
  - `ai_context/AI_CONTEXT.md` — new bullet at top of bullet list (above #6); icon count + total artifact count updated.
  - `ai_context/HISTORY.md` — this entry.
- **Icons authored**:
  - **Icon / Languages** (`228:1684`, key `d8861692ab58281032b9a4d068e38eef8f22e887`) at (868, 1824) — lucide `languages` glyph: stylized "Aa + script-character" composition with diacritical mark over the script char (top-left), and a Latin "A" with cross-bar (bottom-right). Combined into a single VECTOR with 6 subpaths in one path string. Vector frame: x=2, y=2, w=20, h=20. Local path (translated from lucide 24×24 by `(-2, -2)`): `M 3 6 L 9 12 M 2 12 L 8 6 L 10 3 M 0 3 L 12 3 M 5 0 L 6 0 M 20 20 L 15 10 L 10 20 M 12 16 L 18 16`. The user originally said "languages"; the component name `Icon / Languages` matches lucide canonical naming.
  - **Icon / HelpCircle** (`228:1686`, key `72b15496f5485a1780d41165ac510566c788b542`) at (916, 1824) — lucide `help-circle`: circle outline + question-mark curve + dot below. Circle approximated as 4 cubic Beziers (magic-constant `r × 0.5523` with r=10 → control-point offsets of 5.523). Question curve approximated as 2 cubic Beziers combining lucide's `a 3 3 0 0 1 5.83 1` arc + `c 0 2 -3 3 -3 3` descender into one continuous curve. Dot rendered via tiny `M 10 15 L 10.01 15` near-zero line that displays as a circular dot through the `strokeCap = ROUND` (matches lucide's `h .01` pattern). Vector frame: x=2, y=2, w=20, h=20. The user said "help center"; the component name `Icon / HelpCircle` matches lucide canonical naming over the semantic wording for library consistency.
  - **Icon / LogOut** (`228:1688`, key `b02f4d115d30f3a5fb006ed430786a093c5cc0d3`) at (964, 1824) — lucide `log-out`: door frame on the left (rounded rectangle's left half) + horizontal arrow exiting to the right with arrow head. Door frame's 2 rounded corners (top-left and bottom-left) converted from lucide's arc commands (`a 2 2 0 0 1 …`) to cubic Bezier — Figma's vector paths reject SVG arc commands, so each quarter-circle arc of radius 2 becomes a cubic Bezier with control points at distance `r × 0.5523 = 1.1046` from each anchor along the tangent direction. Combined path: door frame (M+L+C+L+C+L) + arrow head (M+L+L) + arrow shaft (M+L) in 3 subpaths. Vector frame: x=3, y=3, w=18, h=18. The user said "exit"; the component name `Icon / LogOut` matches lucide canonical naming over the semantic wording.
- **Library icon count: 25 → 28 lucide icons**. Total reusable artifacts on `❖ Components` page: 23 component sets + 28 lucide icons = **51 reusable artifacts**.
- **Why these three together**: All three close gaps for the Profile screen (`154:2`) — the only working-files frame with multiple inline glyphs deferred since Phase 29 #2. Profile renders 8 list rows (Personal info · Cards · Security · Notifications · Language · Help · About · Sign out), and 3 of them — **Language / Til** (Globe-like glyph in the original inline) · **Help / Yordam** (HelpCircle) · **Sign out / Chiqish** (LogOut) — were the largest pending swap target after Phase 29 #2's deferred list. Now that all 3 glyphs exist as library components, Profile's inline-vector swap is a clean one-shot operation when the user wants to run it.
- **Naming convention reinforced (now applied 2× in two batches)**: User said "languages", "help center", "exit". I defaulted to lucide-canonical names (`Icon / Languages`, `Icon / HelpCircle`, `Icon / LogOut`) per the established library convention. Every existing icon matches its lucide name (Bell, Inbox, ChevronDown, Plus, Send, Search, Lock, Check, X, Delete, ChevronLeft/Right, Home, Menu, User, CreditCard, ShieldCheck, Info, CheckCircle2, AlertTriangle, AlertCircle, Clock, ArrowRight, ArrowDownToLine, Swap = arrow-up-down). User accepted defaults via "ok" both times. **Generalizable rule** (now firm): when user requests an icon with a semantic name that differs from the lucide canonical, default to lucide naming for library consistency. Document the user's semantic wording in the entry's "user said X" prefix so the mapping is searchable.
- **Inline-vector swap deferred (4 targets pending)**: User said "ok" again to the icons-only path; the inline-vector swap on Profile (`154:2`) Globe/Help/LogOut rows + Amount entry (`158:2`) ArrowUpDown was offered but not greenlit. **4 deferred swap targets** now ready for a one-shot swap pass when the user wants to close the loop:
  1. Profile `154:2` "Til / Language" row leading icon → swap to `Icon / Languages` instance
  2. Profile `154:2` "Yordam / Help" row leading icon → swap to `Icon / HelpCircle` instance
  3. Profile `154:2` "Chiqish / Sign out" row leading icon → swap to `Icon / LogOut` instance
  4. Amount entry `158:2` currency-swap inline ArrowUpDown vector → swap to `Icon / Swap` instance (from #6)
- **Verification**: 3 inline screenshots from the build script (each icon at 4× scale via `node.screenshot({ scale: 4 })`). All three render correctly:
  - **Languages**: Aa script-char composition with diacritical mark visible top-left, Latin A with cross-bar bottom-right. Reads as a language-picker glyph at first glance.
  - **HelpCircle**: clean circular outline (no pixelation, smooth cubic-Bezier approximation) + question-mark curve arched cleanly at top with descender + dot below positioned correctly. Reads as the canonical help/support glyph.
  - **LogOut**: door frame on the left with both rounded corners visible (smooth, not faceted) + arrow exiting cleanly to the right with proper 5-5 arrowhead segments + horizontal shaft passing through the door gap. Reads as the canonical sign-out glyph.
- **Build pattern improvements (vs prior single-icon batches)**:
  1. **DRY helper function `makeIconComp(name, x, y, vecX, vecY, vecW, vecH, pathData, strokePaint)`** — factored out the repetitive component+vector+stroke setup. Useful at 3+ icons; for 1-2 icons inline is fine.
  2. **Cubic Bezier rounded-corner conversion at small radius (r=2)** — first time used inside an icon (LogOut door corners). Renders smooth with no visible pixelation. Same magic-constant rule (`r × 0.5523 ≈ 1.1046` control-point offset) used for Bell dome (r=6) and Inbox container (r=2). The rule scales cleanly across sizes.
  3. **Multi-subpath single-VECTOR pattern** — Languages has 6 disconnected subpaths, HelpCircle has 3, LogOut has 3. All represented as one `M…M…M…` path string in a single VECTOR node. No need for multiple VECTOR nodes per icon. Confirmed pattern works at any subpath count.
  4. **Atomic build for related icons** — all 3 icons in one `use_figma` script (within the 10-operation soft limit per the figma-use skill). Atomic semantics mean either all 3 land or none, no partial state. Same pattern would scale to 5-6 icons in one script.
- **No new build pitfalls discovered.** All Phase 29 build conventions held:
  - Foundation Variable binding via `figma.variables.setBoundVariableForPaint` (returns NEW paint, must capture and assign).
  - Component contract: 24×24 frame, `clipsContent = false`, `fills = []`, single VECTOR child with `windingRule: NONZERO`, `strokeWeight: 2`, `strokeCap: ROUND`, `strokeJoin: ROUND`.
  - Vector positioning: vector path data is in LOCAL coordinates (translated from lucide 24×24 viewBox by `(-vecX, -vecY)` so the vector renders inside its frame).
  - Page context: `await figma.setCurrentPageAsync(componentsPage)` at the start of each script (resets between `use_figma` calls).
- **Icons remaining as inline** (still no library glyph yet, deferred to next expansion pass): **Star** (Cards manage default-card indicator) · **Trash** (Cards manage row destructive action) · **Settings** (Cards manage row overflow / kebab-style three-dots). The 3 Cards-manage row glyphs are the only top-of-mind deferreds left from the Phase 28/29 reference frames after #7. With those 3 plus a possible `MoreHorizontal`/`MoreVertical` for kebab affordances, the icon library would be effectively complete for the v1 surface set.
- **What's next** (optional follow-ups, none blocking):
  - (a) **One-shot swap pass** for the 4 deferred targets above (3 Profile rows + Amount entry ArrowUpDown) — atomic operation, replaces inline vectors with icon instances at the same x/y, foundation Variable bindings cascade through;
  - (b) Author **Star + Trash + Settings** (3 final glyphs) to complete the icon-library expansion for the Phase 29 reference frames. Alternative: hold until canonical screen builds need them so the library doesn't grow speculatively;
  - (c) Same canonical screen-build kickoff still pending — pick a marquee surface (Send-money still recommended), draft the spec MD with all 6 generic states + tier_0/1/2 variants + i18n keys + acceptance criteria + state-machine refs, then build to a per-surface page in Figma reusing the now near-complete component-instance + icon-instance vocabulary.
- **Doc cascade**: `docs/product_states.md` (stamp updated to point at #7; Mobile screens row appended w/ full #7 detail) + `ai_context/AI_CONTEXT.md` (new bullet at top + counts updated) + `ai_context/HISTORY.md` (this entry). **No PRD / models / mermaid cascade** — icon authoring is Figma-implementation only; no schema changes, no state machines touched.

---

### 2026-05-05 — Phase 29 follow-up #6 · Icon / Swap + Icon / ChevronDown authored — closes the ArrowUpDown gap deferred from Phase 29 #2 and unblocks future dropdown / expandable affordances

- **Summary**: User asked "make the swap and chevron down icon." Two new lucide icon components authored on `❖ Components` page icon row at y=1824, mirroring the canonical Phase 29 #2/#4/#5 contract: 24×24 frame + single VECTOR child + stroke 2pt ROUND bound to `color/foreground` Variable (`VariableID:15:3`) + no fill on stroke or component frame + vector path in local coordinates so it renders cleanly inside the 24×24 viewBox.
- **Files modified**:
  - Figma file `p8pxXYApMNCpzQn0XkoIw9` — 2 new COMPONENT nodes on `❖ Components` page (`0:1`).
  - `docs/product_states.md` — Last-updated stamp bumped 2026-05-04 → 2026-05-05; Mobile-screens row appended with full follow-up #6 detail.
  - `ai_context/AI_CONTEXT.md` — new bullet at top of bullet list (above Inbox/#5); icon count + total artifact count updated.
  - `ai_context/HISTORY.md` — this entry.
- **Icons authored**:
  - **Icon / Swap** (`226:1580`, key `e339be778260add80634c92b9f73bbc9a23601d9`) at (772, 1824) — lucide `arrow-up-down` glyph: paired up-arrow on the left + down-arrow on the right, each with arrowhead + vertical shaft. Single VECTOR node with multi-subpath data (4 strokes in one path: down arrowhead V + down vertical shaft + up arrowhead inverted-V + up vertical shaft) using `M…L…M…L…` form. Vector frame: x=3, y=4, w=18, h=16. Local path: `M 18 12 L 14 16 L 10 12 M 14 16 L 14 0 M 0 4 L 4 0 L 8 4 M 4 0 L 4 16` (translated from lucide 24×24 source `M 21 16 L 17 20 L 13 16 M 17 20 L 17 4 M 3 8 L 7 4 L 11 8 M 7 4 L 7 20` by `(-3, -4)`).
  - **Icon / ChevronDown** (`226:1582`, key `a57b5bcca19dea3ddd1be39f35c9e604ba7ec43a`) at (820, 1824) — lucide `chevron-down`: clean V pointing down. Mirror of the ChevronLeft / ChevronRight contract. Vector frame: x=6, y=9, w=12, h=6. Local path: `M 0 0 L 6 6 L 12 0` (translated from lucide source `M 6 9 L 12 15 L 18 9` by `(-6, -9)`).
- **Library icon count: 23 → 25 lucide icons**. Total reusable artifacts on `❖ Components` page: 23 component sets + 25 lucide icons = **48 reusable artifacts**.
- **Why both icons in one pass**: Swap closes the explicit `ArrowUpDown` gap left as deferred in Phase 29 #2 (cited as "Amount entry · ArrowUpDown currency-swap icon still inline (no library counterpart)"). ChevronDown is forward-looking — none of the 17 Phase 28/29 reference frames render a dropdown / expandable affordance, but every future dashboard tabbed admin page + every future mobile dropdown / expandable-row pattern will need it, so authoring it once now keeps the icon-library expansion in lockstep with new screen builds and avoids one-off late additions.
- **Inline-swap of Amount entry ArrowUpDown deferred**: User said "ok" to the icons-only path; the inline-vector swap on Amount entry frame (`158:2`) was offered as an extension but not greenlit. Available as a clean one-shot follow-up (locate the existing inline vector → replace with `Icon / Swap` instance at the same position → screenshot verify) when the user wants to close that loop.
- **Verification**: 2 inline screenshots from the build script (each icon at 4× scale via `node.screenshot({ scale: 4 })`) + 3 follow-up 24×24 screenshots via `get_screenshot`. Both render correctly: Swap shows proper paired arrows with arrowheads + shafts at the right scale and orientation; ChevronDown shows the clean V with rounded line caps and the same proportions as ChevronLeft/Right. No layout breakage on the icon row; both occupy the next free 48px-spaced slots after Inbox at x=724.
- **Build pattern reused (no new pitfalls discovered)**:
  1. Inspect canonical icon contract first via read-only `use_figma` — pulled ChevronLeft's exact properties (compW/H, vecX/Y/W/H, strokes binding, vectorPaths, stroke weight/cap/join) and the foreground Variable id (`VariableID:15:3`) to mirror exactly. This dry-run inspection eliminates guessing.
  2. `figma.variables.setBoundVariableForPaint` returns a NEW paint — captured and assigned, never mutated. Helper function `fgStrokePaint()` returns a fresh foreground-bound SOLID paint per call.
  3. Vector frame coordinates are LOCAL to the vector node, not the parent component — translate the lucide 24×24 viewBox path data by `(-vecX, -vecY)` so the vector renders at the right position inside the 24×24 frame. Easy to forget; the inspection of ChevronLeft made it obvious by example (its vector at x=9, y=6 has local path `M 6 12 L 0 6 L 6 0` which is the ChevronLeft `M 15 18 L 9 12 L 15 6` translated by `(-9, -6)`).
  4. Cubic Bezier work was not needed for these icons — pure straight-line glyphs, only `M`/`L` commands. Phase 29 #4 (Bell) and #5 (Inbox) needed cubic Bezier (`C/c`) for accurate circular arcs; Swap and ChevronDown have no curves.
- **Icons remaining as inline** (still no library glyph yet, candidates for next expansion pass): Globe (Profile Til row) · HelpCircle (Profile Yordam row) · LogOut (Profile Chiqish row) · Star (Cards manage default-card indicator) · Trash (Cards manage row destructive action) · Settings (Cards manage row overflow / kebab). With ChevronDown shipped, the only top-of-mind future-need glyph still missing for new surfaces is a small overflow-dot-trio (`MoreHorizontal` or `MoreVertical`) for kebab affordances on list rows.
- **What's next** (optional follow-ups, none blocking):
  - (a) Swap the inline Amount-entry ArrowUpDown vector to the new `Icon / Swap` instance — one-script atomic operation;
  - (b) Promote the 6 remaining inline glyphs to library icons in a single sweep when canonical screen builds need them;
  - (c) Same canonical screen-build kickoff pending from Phase 29 — pick a marquee surface (Send-money still the recommended candidate), draft the full spec MD with all 6 generic states + tier_0/1/2 variants + i18n keys + acceptance criteria + state-machine refs, then build to a per-surface page in Figma reusing the now-fuller component-instance + icon-instance vocabulary.
- **Doc cascade**: `docs/product_states.md` (stamp + Mobile screens row appended w/ full #6 detail) + `ai_context/AI_CONTEXT.md` (new bullet + counts updated) + `ai_context/HISTORY.md` (this entry). **No PRD / models / mermaid cascade** — icon authoring is Figma-implementation only; no schema changes, no state machines touched.

---

### 2026-05-04 — Phase 29 · Working-files frames refactored to component instances — all 17 Auth+Home+Transfer reference screens swapped from inline geometry to instances from `❖ Components`; 2 new component sets authored (Auth top bar + Brand/ZhiMark) to fill gaps

- **Summary**: User asked to "review all pages in working files, replace all elements to existing components from our component library." Surveyed both pages, mapped each inline element on the 17 reference frames (Phase 28) to the closest `❖ Components` library counterpart, authored 2 new component sets where no canonical equivalent existed, then executed per-row batched swaps with screenshot verification. **17 frames refactored · 14 component types instanced · 2 new component sets authored · iOS chrome + Pattern 1 + Processing screen intentionally not swapped.**
- **Why this matters**: Phase 28 frames were pixel-perfect translations of Claude Design JSX — built with inline RECTANGLE / VECTOR / TEXT geometry to match the source. They consumed the foundation **tokens** but not the **components**. Phase 29 closes the loop: every screen now consumes the design system at its highest authoritative level (Components), so future component edits cascade automatically and the frames serve as faithful "what the app looks like when the system is used as authored."
- **Files modified**:
  - `docs/product_states.md` — Last-updated stamp bumped to Phase 29; Mobile screens row appended with full swap log per row + new pitfalls.
  - `ai_context/HISTORY.md` — this entry.
  - `ai_context/AI_CONTEXT.md` — Phase 29 entry refresh + Components count update (20 → 22 sets).
- **Components authored on `❖ Components`** (in single `use_figma` script, then size-fix script for variant `primaryAxisSizingMode`):
  - **Brand / ZhiMark** (`170:223`, key `a78ba8e629700e7046a1b0c39a2d494862f6c255`) — single COMPONENT, 56×56, brand-600 rounded square (`radius/lg` 14pt corner) with Inter ExtraBold "Z" centered (32pt, white, -2% letter-spacing). Position on Components page: (700, 9500). Used on 3 screens (Splash · Lock · would also fit Onboarding-2/3 if those existed).
  - **Auth top bar** (`170:265`, key `f9e7ff1b5ab6466357babbaef8566d3cef17dd5b`) — COMPONENT_SET, 4 variants Step=1|2|3|4. Each variant: 402×48 HORIZONTAL auto-layout with 16pt h-padding, 12pt v-padding, 12pt gap. Left: 24×24 frame containing inline ChevronLeft vector (`M 14.5 6 L 8.5 12 L 14.5 18`, slate-900 stroke 2pt round). Middle: 4 progress bars (each FILL horizontal, 4pt height, `radius/sm` 2pt corner) — bars 1..N filled brand-600, bars N+1..4 filled slate-200. Right: 24×24 frame with inline X vector (slate-400 stroke 2pt). Position on Components page: (100, 10100). Used on 4 Auth screens (Phone · OTP · PIN-Create · PIN-Confirm).
- **Per-frame swap inventory** (every frame on `⏰ Working files`, page `1:5`):
  - **Auth · A · Splash** (`141:3`): inline `Z mark` (64×64) → Brand/ZhiMark (56×56, slight size delta accepted) · inline `Continue CTA` (354×52) → Button Primary Lg with text "Davom etish" via instance text override · ambient glow + ZhiPay tagline + 3 language pills + iOS chrome kept inline.
  - **Auth · B · Phone** (`142:2`): inline `Top bar` (402×48) → Auth top bar Step=1 · inline phone input row (354×60) → Input/Field Variant=Phone State=Default with overrides "+998" / "90 123 45 67" / "Telefon raqamingiz" · inline `CTA` → Button Primary Lg.
  - **Auth · C · OTP** (`142:37`): Top bar → Auth top bar Step=2 · 6 inline OTP boxes (irregular widths) → Input/Otp Length=Six State=Default · inline numpad (354×254) → Input/NumberPad Locale=Comma with **`AmountField.visible = false`** override (the canonical Numpad bundles a 5-million-UZS amount headline that's wrong on non-amount-entry screens; toggling the AmountField child off cleanly reuses the keypad-only).
  - **Auth · D · PIN-Create** (`143:2`): Top bar → Auth top bar Step=3 · numpad → Input/NumberPad with AmountField hidden.
  - **Auth · E · PIN-Confirm** (`143:69`): Top bar → Auth top bar Step=4 · numpad → Input/NumberPad with AmountField hidden.
  - **Auth · F · Onboarding** (`144:2`): inline `Continue CTA` (354×52) → Button Primary Lg "Davom etish" · skip text + lightning illustration + page indicators kept inline.
  - **Auth · G · Lock** (`143:136`): inline `Z mark` (72×72) → Brand/ZhiMark (56×56, centered on the 72-wide footprint via x/y math) · numpad → Input/NumberPad with AmountField hidden.
  - **Home · A · Safe** (`153:3`): inline `Tab bar` (402×88) → Tab bar ActiveTab=Home · inline `KYC banner` (10+204+92 composite) → Banner Variant=Info (renders default "Banner title / Optional body copy" — copy override deferred) · hero CTA card kept inline (custom shape) · saved-recipients pills kept inline.
  - **Home · B · Bold** (`153:90`): Tab bar → Tab bar Home · KYC banner → Banner Info · inline amount-entry card + rate chip + recipient pills kept inline.
  - **Home · C · Profile** (`154:2`): inline `Avatar` (60×60 with "A") → Avatar/Initials Color=Brand Size=Lg with text override "A" · Tab bar → Tab bar ActiveTab=More · 8 list rows (Personal info · Cards · Security · Notifications · Language · Help · About · Sign out) kept inline (no canonical List Row variant exactly matches "icon + label + value/count + chevron" composition).
  - **Home · D · Cards manage** (`154:90`): inline `UC` text mark (15×32) → Scheme logo Scheme=UzCard Size=Sm · inline `HM` text mark (17×32) → Scheme logo Scheme=Humo Size=Sm · card rows + add-card row kept inline.
  - **Transfer · A · Recipient select** (`156:3`): inline search field (354×48 with "Ism yoki Alipay ID") → Input/Field Variant=Search State=Default · 4 inline recipient avatars (48×48) → Avatar/Initials Md with text overrides + color variety: WL Brand · LM Warning · CH Success · ZW Slate · "Yangi oluvchi qo'shish" row kept inline · text-only labels kept inline.
  - **Transfer · B · Add recipient** (`156:60`): 3 inline form fields (74h / 78h / 78h) → 3× Input/Field Variant=Text State=Default · inline `CTA` → Button Primary Lg "Davom etish" · 3 recent-recipient rows kept inline.
  - **Transfer · C · Amount entry** (`158:2`): inline `CTA` (354×52) → Button Primary Lg "Davom etish" · WL recipient header + amount cards + rate chip kept inline.
  - **Transfer · D · Review** (`158:44`): inline `CTA` (354×52) → Button Primary Lg "O'tkazishni tasdiqlash" · big amount headline + recipient/sender card + fee breakdown lines kept inline (Pattern 1 FX Quote Breakdown is canonical pattern, not a swap target).
  - **Transfer · E · Processing** (`159:2`): **fully skipped** — custom 80×80 spinner + title-subtitle composition has no library counterpart. Status Timeline component would represent the canonical state-machine but adopting it is a layout change, not a swap.
  - **Transfer · F · Success** (`159:21`): inline `WL` avatar (44×44) → Avatar/Initials Md Brand "WL" · inline `Yana yuborish` CTA → Button Primary Lg · inline `Asosiy sahifaga` CTA → Button Secondary Lg · success checkmark glyph + recipient summary kept inline.
- **Components on the `⏰ Working files` page used (after Phase 29)**:
  - From `❖ Components` (existing): Button (Primary Lg · Secondary Lg) · Input/Field (Text · Phone · Search) · Input/Otp (Six Default) · Input/NumberPad (Comma) · Tab bar (Home · More) · Banner (Info) · Avatar/Initials (Brand/Warning/Success/Slate × Md, Brand × Lg) · Scheme logo (UzCard/Humo × Sm).
  - From `❖ Components` (new this phase): Auth top bar (Step=1|2|3|4) · Brand/ZhiMark.
- **What stayed inline and why**:
  - **iOS chrome** (status bar with time + cellular bars + battery; dynamic island; home indicator) — library `Status Bar` (`146:1932`) is 375w (iPhone Mini default); working-files frames are 402w (iPhone 15 Pro). Swapping at 375w would leave a 27px gap at the right edge. Decision: keep inline since iOS chrome is device chrome, not product UI.
  - **Pattern 1 — FX Quote Breakdown 6 cells** (`160:3` … `160:138`) — these are themselves canonical pattern artefacts (frames, not components). Skipping per scope decision: this swap pass refactors *screens that consume patterns*, not the pattern source frames.
  - **Onboarding hero illustration**, **Splash ambient glow**, **page indicator dots**, **rate chip on Home B / Amount entry**, **PIN-dot indicators** — no exact library counterpart; would be 1:1 inline matches.
  - **Custom hero CTA cards on Home A / Home B**, **Send card on Home B**, **List rows on Profile**, **fee breakdown lines on Review**, **Processing spinner**, **Success checkmark glyph + recipient summary row** — composition doesn't fit any single component variant cleanly. Promoting these to new component sets is a separate canonical-screen-build decision; this phase explicitly stayed within "swap to existing components" scope.
- **Build pitfalls captured (Figma Plugin API quirks discovered this session)**:
  1. **Auto-layout components shrink to children**: setting `c.layoutMode = "HORIZONTAL"` after `c.resize(402, 48)` changes `primaryAxisSizingMode` to AUTO by default — the component then hug-shrinks to ~56w (sum of children's natural widths). Must explicitly set `primaryAxisSizingMode = "FIXED"` + `counterAxisSizingMode = "FIXED"` AND re-resize after creating variants. Discovered when Auth top bar variants rendered as 56w instead of 402w.
  2. **Numpad component bundles an AmountField headline**: `Input / NumberPad` (`94:152`) was authored for amount-entry screens with a "5 000 000,00 UZS" sample headline above the keypad. On OTP/PIN/Lock screens this is wrong context. Clean override: `npInst.children.find(c => c.name === "AmountField").visible = false` — keeps the canonical instance reference while hiding the inappropriate header. Generalized rule: *components with composite "header + body" shapes should expose their parts via named children so consumers can toggle visibility per screen context*.
  3. **Tree traversal must guard `safe(node, 'children')`**: VECTOR / RECTANGLE / ELLIPSE nodes don't expose `children`, so a recursive `findFirstText(node)` or `findAllText` helper that calls `node.children` directly throws `TypeError: no such property 'children' on VECTOR node`. Always: `const kids = safe(node, "children"); if (!Array.isArray(kids)) return null;`.
  4. **Atomic script failure recovery**: `use_figma` is atomic — when the safe-traversal bug above hit, the entire script aborted with no partial changes. The retry-with-fix pattern is safe: re-run the script, no cleanup needed.
  5. **Variant axis discovery via parsing variant names**: `figma.getNodeByIdAsync(setId).children` gives variants whose `.name` is `"Prop1=Val1, Prop2=Val2"`. Parsing this lets you map the property axes without a separate API. Used to enumerate every component set's prop space upfront.
- **Verification**: 5 `get_screenshot` round-trips (one per Auth frame post-batch + parallel Home batch + parallel Transfer batch). Each frame visually confirmed: component instances render correctly, foundation Variable bindings flow through, no layout breakage.
- **Decisions locked**:
  - **iOS chrome stays inline** until either (a) frames downsize to 393w (iPhone 15 default) to fit `Keyboard & Indicator` 393w + Status Bar 375w-stretchable, or (b) library iOS chrome adds 402w variants for iPhone 15 Pro. Neither is on the urgent path.
  - **Slight visual deltas accepted** when component instance contract (radius / padding / corner / color binding) differs from inline geometry. Example: Splash ZhiMark went from 64×64 → 56×56 (slight shrink), and Cards-manage UC/HM marks went from 15-17w text-marks → ~40w pill marks (Scheme logo Sm). The component's contract wins; if a variant feels wrong, fix the canonical component, not the working-files frame.
  - **Numpad `AmountField` hide pattern** is the new convention for repurposing the component on non-amount-entry screens (OTP, PIN, Lock). Captured in this entry as the canonical workaround until/unless a `WithoutHeader` variant is added to `Input / NumberPad`.
- **Open polish (none blocking)**:
  - Banner instances on Home A/B render default "Banner title / Optional body copy" English copy — override per-screen with localized KYC banner text ("MyID bilan tasdiqlang" + body).
  - Add recipient form fields all show placeholder "Wang Lei" + helper "We'll save this name to your recipients." — override per-field for "Ism / Alipay ID / Eslatma" labels and field-specific helpers.
  - Auth top bar `X` (close) and `<` (back) on each variant are inline vectors at the chevron's exact 6×12 metric — could promote ChevronLeft to a 24×24 lucide icon component on the Components page if back-arrow becomes a recurring need outside Auth.
  - Replace inline iOS chrome with downsized 393w frames + library Status Bar / Keyboard & Indicator instances if a future canonical screen pass commits to iPhone 15 (not Pro).
- **What's next** (optional follow-ups, none blocking):
  - (a) iterate any specific component instance for closer fidelity if a screen surfaces a gap;
  - (b) override Banner / Input field copy per screen for full uz-locale parity;
  - (c) start canonical screen builds — pick a marquee surface (Send-money recommended), draft the full spec MD with all 6 states + tier variants + i18n keys + acceptance criteria, then build to a per-surface page (`📱 Send-money`) reusing the Phase 29 component-instance vocabulary;
  - (d) extend Pattern 1 with the remaining patterns (KYC step · recipient picker · send-money review block · receipt block · transfer-status detail · card-link 3DS).
- **Phase 29 follow-up #5 (same day, 2026-05-04) — Icon / Inbox for empty states**: User asked "make the empty box icon for empty states." First attempt was an `Illustration / Empty state` 180×180 component (slate-100 outer ellipse + open-package shape + flaps + "no items" dots floating above). User interrupted screenshot review with "make the empty box icon for empty states" — clarifying that the request was for a small icon, not an illustration. Deleted the in-progress illustration (`218:1240`) and authored **Icon / Inbox** (`219:1243`, key `9256dacb2a...`, 24×24, lucide-inbox shape) on the icon row at (724, 1824). Single combined path:
  - Tray "U-dip" notch (where mail/items would land): `M 22 12 L 16 12 L 14 15 L 10 15 L 8 12 L 2 12`
  - Outer rounded container — original lucide arcs (`a 2 2 0 0 0 …` for the 4 corners) converted to cubic Beziers using the **magic-constant tangent control points** (radius × 0.5523): `M 5.45 5.11 L 2 12 L 2 18 C 2 19.105 2.895 20 4 20 L 20 20 C 21.105 20 22 19.105 22 18 L 22 12 L 18.55 5.11 C 17.95 4.45 17.36 4 16.76 4 L 7.24 4 C 6.64 4 6.05 4.45 5.45 5.11 Z`
  - 2pt stroke ROUND, foreground-bound (`color/foreground` Variable).
  Available for empty-state usage on list rows / empty history / empty saved-recipients / empty notifications screens. Not yet swapped into any working-files frame because none of the existing Phase 28 frames render an empty state — Recipient select, Profile, etc. all show happy-path data. The icon ships ready-to-use when canonical screen builds add empty-state coverage.
- **Library icon count: 22 → 23 lucide icons**. Total reusable artifacts on `❖ Components`: 23 component sets + 23 lucide icons = **46 reusable artifacts**.
- **Lesson reinforced (3rd time)**: cubic Bezier (`C/c/S/s`) is fully supported in Figma vector paths; only **arc commands (`a/A`) need conversion**. Standard conversion for any quarter-circle arc of radius `r`: cubic Bezier with control points placed at distance `r × 0.5523` from each anchor along the tangent direction. Same trick used for Phase 29 follow-up #4 (Bell dome) and now this Inbox container.
- **Direction-correction precedent**: This is the second case in Phase 29 follow-ups where I started an "illustration" interpretation and the user redirected to a small "icon" interpretation (the first was when authoring the 3 onboarding illustrations — those were correctly illustrations because they ARE the hero element of each slide; this Inbox case is correctly an icon because it's meant to be small and reusable inline). **Generalizable rule**: when the user says "icon for empty state" they mean a 24×24 icon that goes alongside text, not a 180×180 hero illustration. When they say "illustration" they mean the hero treatment. Default to icon size when ambiguous.
- **Doc cascade**: `docs/product_states.md` (stamp + Mobile screens row appended w/ follow-up #5) + `ai_context/AI_CONTEXT.md` (this entry) + `ai_context/HISTORY.md` (full follow-up #5 entry).
- **Icons remaining as inline** (no library glyph yet): Globe · HelpCircle · LogOut · Star · Trash · Settings · ArrowUpDown.

- **Phase 29 follow-up #4 (same day, 2026-05-04) — Icon / Bell authored + bells swapped**: User asked "make bell icon instance." Authored **Icon / Bell** (`209:1140`, key `ddf3bb6cb8...`, 24×24) on the Components page. Initial attempt used Q-curves for the dome (`M 6 8 Q 6 2 12 2 Q 18 2 18 8 …`) but the resulting parabola was too flat to read as a bell — looked like two vertical strokes connected by a small arc. Fixed by switching to cubic Bezier with the standard half-circle approximation: body path `M 6 8 C 6 4.687 8.687 2 12 2 C 15.313 2 18 4.687 18 8 C 18 15 21 17 21 17 L 3 17 C 3 17 6 15 6 8 Z` (the magic constant 0.5523 × radius gives smooth circular arcs from cubic Beziers). Clapper path: `M 10 21 Q 12 23 14 21`. Both paths 2pt stroke ROUND, foreground-bound. Position on icon row: (676, 1824). Swapped both inline bell vectors:
  - **Home A** (`153:20` inside wrapper Frame `153:19` at top-right header) → instance `209:1514` at (358, 21).
  - **Profile** (`154:43` inside wrapper Frame `154:42` for the "Bildirishnomalar" / Notifications row leading icon) → instance `209:1517` at (16, 14).
  Both use the wrapper-frame replacement pattern (entire small wrapper frame replaced by 24×24 icon instance, centered on original wrapper bounds).
- **Library icon count: 21 → 22 lucide icons**. Total reusable artifacts on `❖ Components` page now: 23 component sets + 22 lucide icons = **45 reusable artifacts**.
- **Lesson refined**: the Phase 28 lesson "only M, L, Q, Z accepted in vector paths" was incorrect — only **arc commands** (`a/A`) are unsupported. **Cubic Bezier (`C/c/S/s`) is fully supported** by Figma's `vectorPaths` field. For shapes that need accurate circular arcs (bell domes, lock shackles, scheme-logo circles), use `C` with the magic constant approximation: control points at distance `radius × 0.5523` from anchor along the tangent. Q-curves only approximate parabolas, which are visibly flat for short arcs.
- **Verification**: 3 screenshots — Bell component close-up (renders as proper dome + sides + flared base + clapper), Profile screen ("Bildirishnomalar" row now shows bell instance), Home A (top-right header bell instance).
- **Doc cascade**: `docs/product_states.md` (stamp + Mobile screens row) + `ai_context/AI_CONTEXT.md` (this entry) + `ai_context/HISTORY.md` (full follow-up #4 entry).
- **Icons remaining as inline** (no library glyph yet, candidates for future expansion): Globe (Profile Til row) · HelpCircle (Profile Yordam row) · LogOut (Profile Chiqish row) · Star (Cards manage default-card indicator) · Trash (Cards manage row destructive action) · Settings/three-dots (Cards manage row overflow) · ArrowUpDown (Amount entry currency swap).

- **Phase 29 follow-up #3 (same day, 2026-05-04) — Onboarding illustrations → component set**: User asked "make onboarding icons also reusable icon instances." Inspected the 3 inline 180×180 illustration frames in the onboarding slides (`144:17` Bolt + `189:360` Lock + `189:404` Shield) and promoted them to a single canonical component set on the Components page.
  - **Onboarding illustration** (`204:924`, key `27bdf4e31a...`, COMPONENT_SET, 3 variants `Slide=1|2|3`, each 180×180). Position on Components page: (1300, 10100), wrapper has dashed border + brand-50/950 fill + 16pt corner radius matching the Auth top bar wrapper.
  - **Slide=1 (Bolt)** — outer brand-50 ellipse 148×148 at (16,16) + inner brand-100 ellipse 100×100 at (40,40) + brand-600 bolt vector 58×80 at (62,50), path `M 34 0 L 0 46 L 26 46 L 20 80 L 58 30 L 32 30 L 34 0 Z`. All fills bound to foundation Variables (`color/brand/50` · `color/brand/100` · `color/brand/600`).
  - **Slide=2 (Lock)** — outer brand-50 ellipse + brand-600 lock body rect 60×48 at (60,78) with `radius/sm` 8pt corner + brand-700 shackle U-vector 24×30 at (78,48) — note the **shackle is now correctly positioned above the lock body** (its bottom y=78 meets the lock body's top y=78), instead of the original Slide-2 inline rendering which had the shackle at (0,0) of the frame appearing in the top-left corner. Plus white keyhole dot 10×10 at (85,92) + white stem 4×12 at (88,100). Shackle path: `M 0 30 L 0 12 C 0 4 4 0 12 0 C 20 0 24 4 24 12 L 24 30 L 20 30 L 20 12 C 20 8 16 4 12 4 C 8 4 4 8 4 12 L 4 30 L 0 30 Z`.
  - **Slide=3 (Shield)** — outer brand-50 ellipse + brand-600 shield vector 60×82 at (60,49) **centered** + white check vector 30×22 at (75,80) with 5pt round stroke. Same position-fix as Slide 2: the original Slide-3 inline rendering had shield + check at (0,0) appearing top-left; the canonical component places them at proper centered positions. Shield path: `M 30 0 L 0 12 L 0 42 C 0 54 5 64 15 71 C 20 75 25 79 30 82 C 35 79 40 75 45 71 C 55 64 60 54 60 42 L 60 12 L 30 0 Z`.
  - **Swap mechanic**: for each of the 3 onboarding slides, located the inline 180×180 illustration frame inside its Hero parent, captured the frame's (x,y) position (consistent at x=111, y=0 inside Hero), removed the inline frame, inserted the corresponding variant instance at the same index + position. Variants: `144:17 → 205:1075` (Slide=1) · `189:360 → 205:1079` (Slide=2) · `189:404 → 205:1085` (Slide=3).
  - **Generalizable insight**: when multiple inline frames share a 180×180 footprint with shared decorative chrome (the brand-50 outer circle) and slide-specific content, a single component set with content variants is the cleanest abstraction — even when the variants don't share any geometry beyond the outer circle. Future onboarding additions (slide 4, slide 5, etc.) just need a new variant added to the set. Same pattern as Auth top bar (`Step=1|2|3|4`).
  - **Components count on `❖ Components` page: 22 → 23** (component sets). Total visible artifacts: 23 component sets + 21 lucide icon components = **44 reusable artifacts**.
  - **Verification**: 4 screenshots — 1 of the component set (showing all 3 variants with corrected geometry) + 3 of the swapped onboarding slides (each rendering the variant at native 180×180 inside Hero, with title + body + page-indicator dot + Continue button all preserved).
  - **Doc cascade**: `docs/product_states.md` (stamp + Mobile screens row) + `ai_context/AI_CONTEXT.md` (this entry) + `ai_context/HISTORY.md` (full follow-up #3 entry).

- **Phase 29 follow-up #2 (same day, 2026-05-04) — inline icons → icon instances**: User asked "analyze the icons and remake the do not component instance icons in working files." Inventory pass found **45 inline VECTOR icons** scattered across 14 frames. **30 swapped** to library icon instances; **15 deferred** because the lucide glyph isn't in the library yet (Bell · Globe · HelpCircle · LogOut · Star · Trash · Settings · ArrowUpDown swap) plus 6 onboarding-illustration vectors which are artwork, not icons. **2 new icon components authored** to close the most-used gaps:
  - **Icon / ChevronLeft** (`196:441`, key `74333d3869...`, 24×24, foreground-bound 2pt stroke ROUND, path `M 15 18 L 9 12 L 15 6` — direct mirror of ChevronRight). Used on every back-arrow on the working-files page (5 places).
  - **Icon / Plus** (`198:451`, key `2ff5658f17...`, 24×24, foreground-bound 2pt stroke ROUND, path `M 12 5 L 12 19 M 5 12 L 19 12` — lucide-plus standard). Used on every "Add"-affordance (3 places). Per-instance brand-600 stroke override applied at swap time so the icon reads as a primary action.
  Library icon count: **19 → 21 lucide icons** on the Components page. Both icons added to the icon row at y=1824, x=580 (ChevronLeft) and x=628 (Plus).
- **Per-frame swap inventory** (after this follow-up):
  - **Profile** (`154:2`) — 11 swaps: 7× ChevronRight (one per list row) · Icon/User · Icon/CreditCard · Icon/ShieldCheck · Icon/Info. Bell + Globe + HelpCircle + LogOut still inline (deferred).
  - **Recipient select** (`156:3`) — 6 swaps: ChevronLeft (back) · Search (search-bar wrapper icon) · 4× ChevronRight (recipient row trailing) · Plus (Add row) — Plus added in correction pass after the initial swap mistakenly mapped 156:27 to Search.
  - **Add recipient** (`156:60`) — 4 swaps: ChevronLeft (back) · 3× ChevronRight (recent recipient rows).
  - **Amount entry** (`158:2`) — 1 swap: ChevronLeft (back). ArrowUpDown currency-swap icon still inline (no library counterpart).
  - **Review** (`158:44`) — 2 swaps: ChevronLeft (back) · Lock (rate-lock chip, instance resized to 14×14 to fit the chip's tight padding).
  - **Cards manage** (`154:90`) — 2 swaps: ChevronLeft (back) · Plus ("+ Karta qo'shish" row). Star + Trash + Settings on individual card rows still inline.
  - **Home A — Safe** (`153:3`) — 3 swaps: Send (CTA hero header — paper airplane) · ArrowRight (CTA hero footer) · Plus ("Qo'shish" recipient pill, brand-600 override). Bell on top-right header still inline.
  - **Home B — Bold** (`153:90`) — 1 swap: ChevronRight (in send-card CTA). Other small icons in the send card kept inline (semantic uncertain without label inspection).
  - **Processing** (`159:2`) — 1 swap: Send (paper airplane in spinner core).
  - **Success** (`159:21`) — 1 swap: Check (success indicator).
  - **Onboarding 1/2/3** (`144:2` · `189:345` · `189:389`) — 0 swaps (illustration vectors are artwork, kept inline).
  - **Splash · Phone · OTP · PIN-Create · PIN-Confirm · Lock** — 0 swaps (icons are already inside Auth top bar / Numpad / Input/Field instances from Phase 29).
- **Swap mechanics**:
  - For **VECTOR-inside-small-FRAME-wrapper** cases (e.g., 6×36 back-arrow slots, 22×22 leading-icon slots in Profile rows): replaced the **entire wrapper frame** with a 24×24 icon instance, centered on the wrapper's original position. The instance becomes the new auto-layout slot, sized 24×24.
  - For **direct-child VECTOR** cases (e.g., chevron-right at x=340 inside a 402w list row): aligned the icon's right edge to the original vector's right edge, vertically centered to row mid-line.
  - For **chip-context Lock** (Review rate-lock 9×10 vector): replaced VECTOR with Lock instance + `inst.resize(14, 14)` to fit the chip's compact padding without overflow.
- **Misclassification caught mid-pass**: `156:27` was the "+" Plus icon on the Add-recipient row (`Yangi oluvchi qo'shish`), mistakenly mapped to Search in the first sweep. Caught at screenshot verification. Reverted by removing the misplaced Search instance and replacing with the newly-authored Plus instance, with brand-600 stroke override to match the original "Add" link styling.
- **Foundation Variable bindings preserved**: every icon instance's stroke binds to `color/foreground` by default (inherited from the icon component). Per-instance overrides used only for "Add" affordances (brand-600). Theme switches cascade automatically.
- **Verification**: 7 screenshot round-trips across the most-modified frames (Profile · Recipient select · Cards manage · Home A · Review · Success · Add recipient). All swaps render correctly with proper icon glyphs in the right positions; the Plus-fix pass added 3 more screenshots after the misclassification correction.
- **What's next** (icon library expansion candidates, none blocking): Bell · Globe · HelpCircle · LogOut · Star · Trash · Settings · ArrowUpDown — promote these 8 lucide glyphs to icon components when canonical screen builds need them. Until then they stay inline on the working-files reference frames.

- **Phase 29 follow-up (same day, 2026-05-04) — code-vs-Figma parity gap closed**: User flagged "but there is no all pages in figma that is exists in code." Audit of [`mobile/ZhiPay/*.jsx`](../mobile/ZhiPay/) against the working-files frames found one clear gap: `OnboardingScreen` (`auth-screens.jsx:340`) renders 3 slides driven by `ONBOARDING_SLIDES` — Phase 28 only built **slide 1** (`144:2` "Tezkor o'tkazmalar" + bolt illustration). **Slide 2** ("Aniq kurs" + lock-and-chart illustration) and **Slide 3** ("Xavfsiz va litsenziyalangan" + shield-and-check illustration) were missing. Closed by cloning slide-1 frame twice into `189:345` (slide 2, x=3754) and `189:389` (slide 3, x=4276), then per-clone rebuild of the 180×180 illustration sub-frame (cleared inline children, drew slide-specific shapes from `OnboardingArt(slide)` source) + title/body text override per `ONBOARDING_SLIDES.uz[1]` and `[2]` + page-indicator dot position update (active dot moves to index 1 / 2 — auto-layout grows the 6×6 inactive dot to 24×6 active bar with brand-600 fill bound to `color/brand/600`). Plus 2 section labels above each new frame ("F · Onboarding (slide 2)" · "F · Onboarding (slide 3)"). Slide 3 title needed `textAutoResize = "HEIGHT"` + width=322 to wrap "Xavfsiz va litsenziyalangan" within frame bounds (the Uzbek title is longer than slide 1's "Tezkor o'tkazmalar" so the cloned text node's auto-shrunk width was insufficient). **Frame count on `⏰ Working files`: 23 → 25**. **Other code-vs-Figma deltas explicitly NOT addressed in this pass** (state variants, not separate "pages"): Home `hasRecipients=false` empty state · Home `hasActivity=false` empty state · `kycTier=tier_0/1/2` banner variants · Lock screen with biometric/Face ID button visible on numpad · Profile tier-specific row visibility. These are state branches of existing screens and would warrant their own pass when canonical screen specs include comprehensive state coverage (per the "all 6 generic states + tier variants + i18n keys + acceptance criteria" bar from Phase 28). Doc cascade: stamp · Mobile screens row · this entry.
...
- **Files created**:
  - [`claude-design/brief.md`](../claude-design/brief.md) — 154 lines, ~16 KB. The kickoff message: 1-paragraph product context · 4 personas (PRD §3) · the live Figma file URL with what's already built (33 primitives, 19 semantic aliases light+dark, 13 Text Styles, 8 Effect Styles, 20 component sets, 19 lucide icons — all enumerated by name) · constraints summary (locale uz-first, money in minor units, tier gating, status-machine canonicality, masked PAN privacy, WCAG 2.1 AA) · sequenced task ask (Phase 1 Patterns layer with 10 named patterns → Phase 2 Screens layer with 11 marquee surfaces × 6 states × tier variants → Phase 3 Flows layer wired against canonical mermaid) · output expectations (frame + spec MD + which tokens / primitives / components / patterns instanced + states / tiers / locales drawn + i18n keys referenced + deviations) · 12-row design-review checklist · "ask before inventing" instruction.
  - [`claude-design/rules-pack.md`](../claude-design/rules-pack.md) — 810 lines, ~32 KB. The reference attachment: 8 distilled rules (core principles · money & FX rendering · KYC tiers & limits · status machines with canonical mermaid diagrams · error UX · card schemes · localization · accessibility) · §9 design review checklist · §10 layer hierarchy (Tokens → Primitives → Components → Patterns → Screens → Flows + import-direction matrix) · §11 doc slices (PRD §3 personas + §6.1 mobile features + models §2.2 KYC tier definitions + §3.2 card schemes + §4.3 FX rate lock invariant + §3.3 wallet ledger + §7.1 canonical 15 error codes + §11.8 destination identifiers) · §12 canonical mermaid sequence flows (onboarding · card linking · transfer send) · §13 glossary (MyID / PINFL / tiyin / fen / acquirer / provider / 3DS / FX rate lock / tier_0/1/2 / error_codes) · §14 "what to build with what" cheat sheet (32 design contexts → exact component / variant / size to reach for).
- **Master prompt drafted in conversation** (separate from the file artifacts) — the user's third request. Sets aesthetic direction: clear · clean · elegant · modern · minimalistic. Reference bar: Wise / Revolut / Cash App / Robinhood / Linear (2024–2026 fintech). 10 enforced principles (single focal point per screen · generous whitespace · typography heavy lifting · color used sparingly · soft considered elevation · functional motion · stroke icons at 2pt · intentional numbers · 8/12/20pt rounded corners · trust through transparency) + an explicit "do NOT" list (no gradients on chrome · no drop shadows on text · no decorative illustrations · no emoji as iconography · no borders everywhere · no centered body text in long-form · no mixed icon styles · no crowded layouts) + working cadence (one artifact at a time, pause for review) + first-move instruction (read rules-pack → read brief → read live Figma → propose Pattern 1 FX-quote breakdown).
- **No PRD / models / mermaid / state-machine cascade** — the pack is a *projection* of existing authoritative facts onto an external tool's context window, not a new authoritative source. If anything in the rules-pack drifts from `docs/` or `.claude/rules/` after future doc edits, the rules-pack is the one that needs re-syncing — never the other way around. Designs produced inside Claude Design will need to come back through this repo's `mobile/design/patterns/` and `mobile/design/screens/` paths to become canonical.
- **Decision locked**: the `claude-design/` folder is for external-tool kickoff artifacts only. It is NOT a substitute for `mobile/design/` (which remains the durable spec source of truth). Any new external tools that need a similar pack drop a sibling MD here following the same brief + rules-pack split.
- **Doc cascade scope**: `ai_context/AI_CONTEXT.md` (added `claude-design/` to the "Where things live" tree with a one-line description of each file) + `ai_context/HISTORY.md` (this entry). No `docs/product_states.md` change — no surface or foundation row flipped status; no schema / state machine / flow / mindmap change; the mobile design system foundation in Figma is unchanged from the previous entry.
- **Files modified**: `ai_context/AI_CONTEXT.md` · `ai_context/HISTORY.md` (this entry).
- **Files created**: `claude-design/brief.md` · `claude-design/rules-pack.md`.
- **What's next**: User uploads the two files to their Claude Design canvas and pastes the master prompt as the kickoff message. Output from Claude Design (patterns / screens / flows) flows back into `mobile/design/patterns/`, `mobile/design/screens/`, `mobile/design/flows/` for canonicalization — no current ETA. Optional follow-ups still open from the previous entry: (a) replace Scheme Logo placeholder marks with licensed brand SVG/PNG when they land; (b) add Tab Bar's optional CountBadge slot via INSTANCE_SWAP; (c) Smart Animate prototype connections for sheet snap / modal open / timeline pulse; (d) Input hidden leading/trailing icon slots on Text/TextArea/Phone variants for full Button-parity; (e) sample app shell stitching primitives + components in light / dark / 200% dynamic-type as a foundation smoke-test.

---

### 2026-05-04 — Mobile Pass 2 · Components layer COMPLETE in Figma — 7 remaining component sets BUILT: Toast (4) · Segmented Control (6) · Tab Bar (4) · Sheet (3) · Modal (3) · Status Timeline (4) · Scheme Logo (12) = 36 cells across 7 sets · 2 new lucide icons (Home · Menu) · all 11 component spec MDs now have Figma sections

- **Summary**: Closed the Components layer in Figma in a single sweep. All 7 remaining components (Modal · Scheme Logo · Segmented Control · Sheet · Status Timeline · Tab Bar · Toast) built and their `## Figma component-set` sections appended to their spec MDs. Combined with the 4 Components built in Phase 26 (Card · Banner · Headline Number · List Row), the Components layer is now **fully covered in Figma — all 11 component sets live**.
- **Files modified**:
  - `mobile/design/components/toast.md` — Figma component-set section appended (single Variant axis, 4 tonal cells)
  - `mobile/design/components/segmented-control.md` — Figma section appended (Variant × Size = 6 cells, 3-segment fixed)
  - `mobile/design/components/tab-bar.md` — Figma section appended (single ActiveTab axis, 4 cells, prerequisite icons documented)
  - `mobile/design/components/sheet.md` — Figma section appended (SnapPoint axis, 3 progressive heights with sample content)
  - `mobile/design/components/modal.md` — Figma section appended (Variant axis, 3 full-viewport cells)
  - `mobile/design/components/status-timeline.md` — Figma section appended (Variant axis, 4 terminal-state cells)
  - `mobile/design/components/scheme-logo.md` — Figma section appended (Scheme × Size = 12 cells; Visa/MC excluded per LESSON 2026-04-30)
  - `ai_context/HISTORY.md` — this entry
  - `ai_context/AI_CONTEXT.md` — Phase 27 entry refreshed (Pass 2 progress: Primitives + Components layers both COMPLETE; sample app shell + Patterns / Screens / Flows next)
- **Files created in Figma** (file `p8pxXYApMNCpzQn0XkoIw9`, page `❖ Components`):
  - **2 new lucide icon components** (now 19 icons total): `Icon / Home` (`114:143`) at (484, 1824) — for Tab Bar Home tab; `Icon / Menu` (`114:148`) at (532, 1824) — for Tab Bar More tab. Both authored via `figma.createNodeFromSvg`, stroke-bound to `color/foreground`, 24×24 baseline.
  - `Toast` — `110:160` at (100, 7028), 500×520. **4 cells** (Variant=Success/Info/Warning/Error). Each cell: `bg-card`-style fill replaced with variant-50 surface (success/warning/danger) or slate-100 (info), 3pt left accent in variant-600 via strokeLeftWeight, `effect/shadow-md/light`, 380pt fixed width × hug height, icon + title + body composition. Icons via existing CheckCircle2 / Info / AlertTriangle / AlertCircle.
  - `Segmented control` — `112:182` at (700, 7028), 1260×248. **6 cells** (Variant=InlineFlex/FullWidth × Size=Sm/Md/Lg). Pill-shaped tracks bound to `color/muted`, `radius/pill`, with active segment as a thumb (`color/card` + `effect/shadow-sm/light`). 3-segment "Recent / Saved / New" baked in every cell with first segment active. FullWidth variant uses `layoutGrow=1` per segment.
  - `Tab bar` — `115:228` at (100, 7600), 480×480. **4 cells** (ActiveTab=Home/Send/History/More). 360pt × 56pt mobile tab bar with 1pt top border, 4 tabs in flex-1 distribution. Active tab: brand-600 icon + Semibold label; inactive: slate-400 icon + slate-500 Medium label. Active dot indicator + scale animations deferred (animation territory).
  - `Sheet` — `118:251` at (700, 7600), 1264×820. **3 cells** (SnapPoint=Peek 240 / Half 480 / Full 700). Each shows the canonical chrome: 36×4 drag handle bar + (Half/Full) header with title + close X + content area with 1/3/5 sample recipient rows + (Half/Full) sticky bottom action with Continue button. Top corners `radius/lg` (20pt); `effect/shadow-lg/light`.
  - `Modal` — `120:236` at (100, 8500), 1384×880. **3 cells** (Variant=Confirm/Destructive/Info). Full mobile viewport (360 × 760) per spec — no scrim, no centered card. Each cell: top-left close X + centered 56pt icon + title (`text/heading` 22/600) + body + bottom CTA stack. Confirm = brand primary + secondary; Destructive = danger primary + Cancel; Info = brand primary only. **Initial layout had body height 540 + CTA stack auto-hugging exceeding the 760pt viewport**, clipping CTAs at the bottom. Fixed by setting body height to `760 - headerH - ctaStackH` and forcing `ctaStack.layoutSizingVertical='HUG'`. Lesson: `resize(_, 0)` on auto-layout containers can stick despite sizing-mode declarations; force HUG explicitly.
  - `Status timeline` — `128:327` at (700, 8500), 1496×520. **4 cells** (Variant=Processing/Completed/Failed/Reversed). Vertical event stream with 12pt circles + 56pt connecting lines per row. Three circle states (past-filled, current-ring, future-hollow) bound to state-tone-600 / slate-300. Sample copy: "Created · Auth captured · Sent to Alipay · Confirming…" for Processing; full success path for Completed; "Failed" terminal with `CARD_DECLINED` meta for Failed; success path + warning Reversed for Reversed. **Initial cells collapsed to 0pt height** — same `resize(_, 0)` pitfall as Modal; fixed via `layoutSizingVertical='HUG'`. **Current-event ring with inner dot simplified to ring-only** (auto-layout doesn't support overlay positioning); **dashed line approximated via 0.4 opacity on solid line** (Figma RECTANGLE fills don't accept stroke-dash). Both deviations tracked.
  - `Scheme logo` — `132:217` at (100, 9500), 510×440. **12 cells** (Scheme=UzCard/Humo/Alipay/WeChat × Size=Sm/Md/Lg). **Visa / Mastercard variants NOT built** per [LESSON 2026-04-30](LESSONS.md). Brand-color placeholder marks: UzCard `#0066B3`, Humo `#F58220`, Alipay `#1677FF`, WeChat `#07C160` — white Inter Bold wordmark inside. Brand colors hardcoded (not token-bound) since brand colors don't have semantic equivalents in the foundation; matches the admin dashboard's existing placeholder pattern. UzCard / Humo use `radius/sm` (8pt — card mark); Alipay / WeChat use `radius/pill` (round wallet marks).
- **Spec sections appended in identical rhythm** to the Primitives layer pattern: Variant axes → Naming → Variable bindings → File placement → Deviations table. Each section runs ~80–110 lines with explicit recovery paths for every deviation.
- **2 build pitfalls discovered + documented** (both involving `resize(_, 0)` on auto-layout containers):
  1. **Modal CTA clipping** — fixed-height parent + auto-hug child where parent's `clipsContent: true` cuts off the child if the static body height stack exceeds the parent height
  2. **Status Timeline cells collapsed to 0pt** — `resize(280, 0)` stuck despite `primaryAxisSizingMode: 'AUTO'`; required explicit `layoutSizingVertical: 'HUG'` post-resize
  Both lessons captured in their respective deviations tables. **Generalizable rule**: when stacking auto-layout children inside a parent, force `layoutSizingVertical/Horizontal: 'HUG'` explicitly post-resize rather than relying on `primaryAxisSizingMode` to recompute from `resize(0)`.
- **Verification**: 7 inline `await screenshot()` round-trips across the build, one per component set. All 36 cells render correctly with proper colors, geometry, typography. Two follow-up screenshots after the Modal layout fix and the Status Timeline height fix confirmed the layout corrections landed.
- **Pass 2 progress — Components layer COMPLETE**: 11 of 11 component sets in Figma (Card · Banner · Headline Number · List Row · Toast · Segmented Control · Tab Bar · Sheet · Modal · Status Timeline · Scheme Logo). Combined with 5 Primitives complete, the **mobile design system foundation in Figma is now fully built** — Tokens (locked Phase 23), Primitives (locked Phase 27c), Components (locked this session). 19 lucide icon components live in the icon row.
- **Total component sets on the `❖ Components` page**: **20** (4 Phase 26 Components + 5 Pass 2 Primitives + 11 Components layer = 20 component sets) plus 19 individual Icon components.
- **Lessons applied / surfaced this session**:
  - 2026-04-29 typography 13pt floor (Tab Bar labels at `text/label` 13pt with chip-style override)
  - 2026-04-30 Visa/Mastercard scope-out (Scheme Logo authored only 4 of 6 schemes)
  - 2026-05-01 buttons + flowing meta never below 14pt (Toast title/body, Modal title/body, Sheet copy all 14pt+ minimum)
  - 2026-05-02 Title Case (Tab Bar labels Title Case as authored — chip-style textCase override)
  - 2026-05-03 fixed-bottom mobile action bars edge-to-edge consistency (Segmented Control's `FullWidth` variant — exact root cause of the lesson)
  - **NEW (this session)**: `resize(_, 0)` + `primaryAxisSizingMode: 'AUTO'` doesn't reliably recompute height; force `layoutSizingVertical: 'HUG'` explicitly. Captured in Modal + Status Timeline deviations.
- **Open items**: All 36 cells have documented deviation paths — none blocking. The most-warranted follow-ups are: (1) replace Scheme Logo placeholder marks with real licensed brand SVG/PNG assets when they land; (2) add Tab Bar's optional CountBadge slot via INSTANCE_SWAP when notification surfaces require it; (3) Smart Animate prototype connections for sheet snap transitions, modal open, and timeline pulse — all animation territory deferred from static frame builds.
- **What's next**: With both Primitives and Components layers fully built in Figma, Pass 2 is complete. Natural next steps: (a) sample app shell stitching primitives + components in light / dark / 200% dynamic-type per the foundation brief; (b) Patterns layer — KYC step / FX-quote breakdown / send-money review block / etc.; (c) Screens layer — Onboarding / Send-money / History / Receipt / etc.; (d) Optional Input follow-up (hidden leading/trailing icon slots on Text/TextArea/Phone for full Button-parity).

---

### 2026-05-04 — Mobile Pass 2 · Avatar + Icon primitives — closes the Primitives layer in Figma · 18 Avatar cells (Initials 15 + Photo 3) + 17-icon library showcase frame + avatar.md and icon.md "Figma component-set" sections appended

- **Summary**: Closed out the Primitives layer in Figma by building the last two primitives — Avatar (18 cells across 2 component sets) and Icon (no component set; the 17-component lucide library + a new full-library showcase frame). **All 5 mobile primitives now have their Figma sections** (Button + Chip + Input + Avatar + Icon). Pass 2 for the Primitives layer is **complete**.
- **Files modified**:
  - `mobile/design/primitives/avatar.md` — appended ~155-line `## Figma component-set` section before `## Cross-references`. Two-set structure (Initials + Photo) with deferred status-dot + selected-ring affordances documented.
  - `mobile/design/primitives/icon.md` — appended ~110-line `## Figma component-set` section. Documents the 17-icon library inventory (with node IDs + lucide names + which phase added each), the convention "Icon is the only primitive without a variant component set" (each icon is its own discrete COMPONENT), and the two showcase frames (canonical 8 from Phase 26 + the new full 17 from Pass 2).
  - `ai_context/HISTORY.md` — this entry.
  - `ai_context/AI_CONTEXT.md` — Phase 27 entry refreshed (Pass 2 progress: 5 of 5 primitives done; Components layer next).
- **Files created in Figma** (file `p8pxXYApMNCpzQn0XkoIw9`, page `❖ Components`):
  - `Avatar / Initials` — component set `105:116` at (100, 6308), 480×620. **15 cells = 5 colors × 3 sizes**. Colors `Brand · Slate · Success · Warning · Fallback` map to the 4 deterministic name-hash buckets per spec (`firstChar(md5(name)) → bucket`) plus the `slate-100` empty/unknown fallback. **No `Danger` (red) avatar** — per spec line "Never derive a danger-600 avatar — red is reserved for failure semantics." Sizes `Sm` 32 / `Md` 48 / `Lg` 72 with `text/label` (Sm) · `text/body-sm-semibold` (Md) · `text/heading` (Lg) typography. Sm size carries a layer-level `Inter Semi Bold` weight override (foundation `text/label` is Medium). All 15 cells centered via auto-layout `primaryAxisAlignItems=CENTER` + `counterAxisAlignItems=CENTER`. Demo content: `WL` (Wang Lei) at every cell.
  - `Avatar / Photo` — component set `105:129` at (700, 6308), 480×220. **3 cells = `Sm · Md · Lg`**. Each cell renders a placeholder: `color/slate/300` surface + `Icon / User` instance centered at 0.55× avatar diameter (18 / 26 / 40pt) with `color/base/white` stroke override. Designers attach actual photos at instance time by switching the container's fill type from Solid → Image. `clipsContent=true` masks images to the circle. Image-error fall-through to initials variant is a runtime concern (not authored in Figma).
  - **`Icons (17 — full library)`** showcase frame `108:92` at (100, 1900), 724×244. **6×3 grid** of all 17 lucide icon components with their names labeled below in `text/body-sm` slate-500. Each cell carries an instance of the corresponding icon at 20pt with stroke bound to `color/foreground` (slate-700 light / slate-300 dark). Sits below the second icon row (y=1824) without disturbing the existing `Icons (canonical 8)` showcase at (100, 1700) — both showcases coexist; the canonical-8 stays as the foundation-prompt reference, the full-17 is the discoverable inventory designers consult.
- **No Figma component set for Icon** — design choice documented in icon.md: each lucide icon is its own discrete COMPONENT (matches lucide-react's per-icon import shape). Authoring a multi-axis component set with all 17 icons would force designers to swap variants every time they want a different glyph. The library grows on-demand; new icons get appended to the showcase + the spec table.
- **Component properties (deferred for Avatar)**:
  - **Status dot** — spec line "Mostly unused at v1" honored. Recovery path documented: add `Show status dot` BOOL + `Status` (Online/Offline) VARIANT to both Avatar sets when support-chat / agent-online surfaces land.
  - **Selected ring** (recipient multi-select) — handled at instance time via outset stroke override on the avatar container. No component property needed.
- **Verification**: Two `await screenshot()` round-trips:
  1. **Avatar / Initials + Avatar / Photo**: 15-cell Initials grid showed brand/slate/success/warning/fallback color progression × 3 sizes, all with WL initials in semibold; 3-cell Photo grid showed slate-300 placeholder with white User icon scaling 0.55× per size.
  2. **Icons (17) showcase**: confirmed all 17 icons (Send through Lock) render at 20pt slate-700 with name labels in slate-500.
- **Lessons applied**: 2026-04-29 typography 13pt floor (Sm initials at `text/label` 13pt; chip-style override on textCase + letterSpacing) · 2026-04-30 Visa/Mastercard scope-out (no card-scheme glyphs in the icon library; scheme logos go to the Components-layer `SchemeLogo` primitive per chip-schemes precedent) · 2026-05-02 Title Case (avatar initials are uppercase by content `WL`, not by Style override; foundation textCase override applied where text/label baked uppercase).
- **Pass 2 progress — Primitives layer COMPLETE**: 5 of 5 primitives now have their Figma sections in their spec MDs (Button + Chip + Input + Avatar + Icon). All 4 + 5 = 9 component sets live in the file (`Button` + `Chip / Status` / `Tier` / `Count` + `Input / Field` / `Otp` / `NumberPad` + `Avatar / Initials` / `Photo`) plus the 17-icon library + 4 Components-layer sets from Phase 26 (Card · Banner · Headline Number · List Row). **Total component sets on the page: 13.**
- **Open items**: Avatar deferred deviations (status dot, selected ring) — both with recovery paths. Icon library is 17 of ~1500 lucide icons — grows on-demand. **Next**: either Components-layer Pass 2 (the 7 remaining components — Modal · Scheme Logo · Segmented Control · Sheet · Status Timeline · Tab Bar · Toast) OR the optional Input follow-up (add hidden leading/trailing icon slots to Text/TextArea/Phone variants per the Button pattern). User to direct.

---

### 2026-05-04 — Mobile Pass 2 · Input primitives BUILT in Figma — `Input / Field` (12) + `Input / Otp` (6) + `Input / NumberPad` (2) = 20 cells across 3 component sets + 4 new lucide icons (Search · X · Delete · Lock) + input.md "Figma component-set" section appended

- **Summary**: Continued Pass 2 with the most complex primitive in the layer — Input. Where the chip spec packed three small sub-types into ≤ 30-cell sets, Input has 7 spec variants × 6 states each, which would explode into hundreds of cells if authored as a full matrix. Mirroring Button's pattern of dropping spec axes that bloat the matrix without instance-time benefit, the build splits Input into **3 discrete component sets** with conservative axis counts and handles the rest via instance overrides.
- **Files modified**:
  - `mobile/design/primitives/input.md` — appended ~250-line `## Figma component-set` section before `## Cross-references`. Same rhythm as `button.md` and the freshly-shipped chip section: prerequisite icon list → per-set Variant axes / Naming / Variable bindings → effect handling → skip cells → deviations table → file placement.
  - `ai_context/HISTORY.md` — this entry.
  - `ai_context/AI_CONTEXT.md` — Phase 27 entry refreshed (Pass 2 progress: Button + Chip + Input done; Avatar + Icon next).
- **Files created in Figma** (file `p8pxXYApMNCpzQn0XkoIw9`, page `❖ Components`):
  - **4 new lucide icon components** appended to the icon row at y=1824 (x=292/340/388/436, 48pt spacing matching the existing 13 icons): `Icon / Search` (`86:73`) for the Search field magnifier; `Icon / X` (`86:77`) for the Search clear button + future modal close; `Icon / Delete` (`86:82`) for the NumberPad delete key; `Icon / Lock` (`86:86`) for read-only field state + recovery for the `Frozen` Chip status (deviation logged in chip.md). All four authored via `figma.createNodeFromSvg` at 24×24, stroke-width 2, default-bound to `color/foreground` — same pattern as the foundation's original 13 icons.
  - `Input / Field` — component set `90:154` at `(100, 4500)`, 1200×656. **12 cells = 4 variants × 3 states**. Variants: `Text` (single-line, leading/trailing icon slots wired for instance-time use), `TextArea` (96pt min-height, multi-line value, top-aligned), `Phone` (locked `+998` prefix box at 72×48pt with `bg-muted` + 1pt right border + `text/body` Medium override at slate-500, then auto-grow main input area), `Search` (leading `Icon / Search` 20pt slate-400 baked + trailing `Icon / X` 16pt slate-500 visible only when `State≠Default`). States: `Default` (slate-400 placeholder, 1pt `color/input` border), `Focused` (slate-900 value, 2pt `color/ring` border), `Error` (slate-900 value, 2pt `color/destructive` border, danger-600 Medium-weight error text). All cells share label/field/helper vertical rhythm with `space/2` gap, 328pt fixed width, `radius/sm` 8pt corners, `text/body` 16pt for value (16pt floor prevents iOS auto-zoom), `text/body-sm-medium` for label, `text/body-sm` for helper.
  - `Input / Otp` — component set `92:134` at `(1400, 4500)`, 1350×312. **6 cells = 2 lengths × 3 states**. Length: `Four` (MyID confirm) · `Six` (SMS, default). State: `Default` (filled boxes get slate-300 1pt border, empty boxes get color/input 1pt border — soft "this digit is set" cue), `Focused` (next-empty box gets 2pt `color/ring` border, others stay idle), `Error` (all boxes get 2pt `color/destructive` border — not just one, per spec). Per-box: 48×48pt, `space/2` (8pt) gap between boxes, `radius/sm` 8pt corners. Digit text: `Inter Semi Bold` (600), 22pt, line-height 28pt, slate-900, tabular-nums. Demo content: 4-box variants show `1 2 _ _` (2 filled), 6-box variants show `1 2 3 4 _ _` (4 filled). PIN variant deferred per spec ("Not used in v1").
  - `Input / NumberPad` — component set `94:152` at `(100, 5300)`, 680×540. **2 cells = Locale=Comma · Locale=Period**. Each cell = vertical stack of: faux Headline-Number AmountField (text/display-2 32/700 slate-900 value + text/body 16/500-override slate-500 currency code) + 4×3 keypad grid. Keypad cells: 72×72pt, `color/card` fill, 1pt `color/input` border (added to disambiguate white-on-near-white at the page-background level — flagged as deviation), `radius/sm` 8pt corners, 4pt grid gap, digit text `Inter Medium` 22pt slate-900. Per-row content: `1 2 3` / `4 5 6` / `7 8 9` / `<decimal> 0 <Delete glyph>`. Decimal flips per Locale (`,` for Comma uz/ru, `.` for Period en); integer separator in the AmountField also flips (`5 000 000,00` Comma vs `5,000,000.00` Period). Delete key: `Icon / Delete` instance at 24pt, slate-700 stroke override.
- **Spec-vs-build deviations tracked** (6 in input.md "Deviations from spec, tracked" table):
  1. `Disabled` and `Read-only` states dropped from `Input / Field` matrix → handled via instance-time overrides (matches Button's State-axis-deferred pattern).
  2. `PIN` variant not built — slated for v1.1+ per spec.
  3. `Focused` state uses single 2pt border instead of the spec's "2pt ring + 2pt offset" Effect-Style stack (simpler visual contract on small mobile fields; upgrade if `prefers-contrast: more` reveals issues).
  4. OTP digit weight via `fontName: 'Inter Semi Bold'` layer override (no 22pt Semibold Style exists; could migrate to `text/heading` 22/600 in a follow-up — that style exists and matches).
  5. NumberPad keys carry a 1pt `color/input` border (spec calls for `bg-card` only; needed for visibility against the page-background; harmless on real bottom-sheet contexts).
  6. Phone prefix uses `text/body` (Regular) with `Inter Medium` layer override (no Style at this size+weight+secondary-color combo).
- **Verification**: Three `await screenshot()` round-trips during the build:
  1. **NumberPad first attempt** revealed the white-on-near-white invisibility + amount field clipping → fixed in a follow-up call (added 1pt borders to all 24 keys via `set.children → Keypad → row → key` traversal; switched amount fields from fixed-width to `layoutSizingHorizontal=HUG`).
  2. **All 3 sets verified individually**: Field showed all 12 state×variant cells rendering with correct colors, borders, and content; Otp showed 6 cells with correct fill-vs-empty box differentiation and per-state border treatment; NumberPad showed both Locale variants with correct decimal glyph + integer separator flips.
- **Page placement** (lesson from chip build applied): every build script started with `await figma.setCurrentPageAsync(componentsPage)` before any `figma.combineAsVariants(...)` or `figma.createNodeFromSvg(...)` call. No misplacement this round — all assets landed on `❖ Components` directly.
- **Pass 2 progress**: `3 of 5 primitives` now have their Figma sections (Button + Chip + Input). Remaining: **Avatar** (3 sizes × {initials, photo} + status-dot — small, ~6–9 cells) · **Icon** (likely a frame-index / showcase frame for the now-17-icon library, not a true variant set — confirm shape with user before authoring).
- **Lessons applied**: 2026-04-29 typography 13pt floor (helper text `text/body-sm` 14pt; value text 16pt minimum to dodge iOS auto-zoom — explicitly documented in spec) · 2026-05-01 buttons + flowing text never below 14pt (helper, label, error all 14pt) · 2026-05-02 Title Case (Phone prefix `+998`, all labels Title Case as authored — no uppercase tracking-wider on input chrome).
- **Open items**: 7 deviations tracked in input.md table — none blocking, all with documented recovery paths. The OTP `text/heading` migration (deviation #4) is the strongest candidate for a one-line follow-up. Pass 2 continues with **Avatar** next per the user-approved sequence.
- **Post-build polish (same session, user-driven)**: (1) **Layout restacked vertically** — initial build placed `Input / Otp` side-by-side with `Input / Field` at (1400, 4500) and `Input / NumberPad` at (100, 5300); user flagged the unbalanced layout. Repositioned to a vertical stack at x=100 with 100pt gaps: `Input / Field` (100, 4500) → `Input / Otp` (100, 5256) → `Input / NumberPad` (100, 5668). Matches the down-the-page rhythm Card / Banner / Headline Number / List Row / Button / Chip already follow. (2) **`INSTANCE_SWAP` component properties wired** so icons can be swapped at instance time (matches Button's `Leading icon` / `Trailing icon` properties). Three properties added: `Input / Field` → `Leading icon` (default `Icon / Search`) + `Trailing icon` (default `Icon / X`); `Input / NumberPad` → `Delete icon` (default `Icon / Delete`). All three properties carry `preferredValues` listing all **17 lucide icons** so the right-panel picker surfaces the full set. Wired on 5 Search-variant instance nodes (leading on Default/Focused/Error; trailing on Focused/Error) + 2 NumberPad Delete cells (one per Locale). **Key API note**: INSTANCE_SWAP defaults + `preferredValues` use **node IDs** (e.g. `42:30`) for local-only components, not the 40-char published library `.key` (e.g. `42b2fbd4cc31fa3...`). First attempt with `.key` failed with "Property value is incompatible"; switched to `.id` per the Button precedent's stored property-definition values. (3) **Spec section updated** in `input.md`: new "Component properties — icon swap" subsection documents the 3 properties + how they were wired; "File placement" table reflects new positions; "Deviations from spec, tracked" table grew from 6 to 7 entries (Text/TextArea/Phone variants having no icon slots is the new entry — follow-up path documented: add hidden leading + trailing instances + Boolean toggles + reuse the existing INSTANCE_SWAP properties).

---

### 2026-05-04 — Mobile Pass 2 · Chip primitives BUILT in Figma — `Chip / Status` (30) + `Chip / Tier` (9) + `Chip / Count` (18) = 57 cells across 3 component sets + chip.md "Figma component-set" section appended

- **Summary**: Continued Pass 2 (Figma component-set construction from primitive specs). After `button.md` shipped its Figma section yesterday, the remaining 4 primitive specs sat without their Figma sections. Built all three Chip primitives — `StatusChip`, `TierBadge`, `CountBadge` — as **three discrete component sets** under a shared `Chip / *` namespace. The decision to split into three sets (rather than one mega-axis set with `Type=Status|Tier|Count`) matches the asset-panel rhythm Card / Banner / Button already established (≤ 30 cells per set, clear semantic grouping in the Assets palette, no wasted skip-cells).
- **Files modified**:
  - `mobile/design/primitives/chip.md` — appended ~210-line `## Figma component-set` section before `## Cross-references`. Mirrors `button.md`'s rhythm (Variant axes → Naming → Variable bindings per cell → Effect → Skip cells → Deviations → File placement). Three sub-sections, one per component set.
  - `ai_context/HISTORY.md` — this entry.
  - `ai_context/AI_CONTEXT.md` — current-phase + active workstreams refreshed (Pass 2 progress: Button + Chip done · Avatar / Input / Icon next).
- **Files created in Figma** (file `p8pxXYApMNCpzQn0XkoIw9`, page `❖ Components`):
  - `Chip / Status` — component set `71:92` at `(100, 3720)`, 510×680. 30 cells = 10 statuses (`Created` · `Processing` · `Completed` · `Failed` · `Reversed` · `Pending` · `Passed` · `Expired` · `Active` · `Frozen`) × 3 sizes (`Sm` 24pt · `Md` 28pt · `Lg` 32pt). Tones dedupe across domains: `Failed` and `Expired` are the same chip whether applied to a transfer / KYC / card row (consumer disambiguates by row context). Spec maps 5 surface palettes (slate / brand / success / warning / danger) across the 10 cells.
  - `Chip / Tier` — component set `75:29` at `(700, 3720)`, 510×288. 9 cells = 3 tiers × 3 sizes. `Tier2` carries the MyID glyph baked into the cell (not a separate axis — per chip.md "Tier_2 badge without the MyID glyph" forbidden). **Glyph deviation tracked**: `Icon / BadgeCheck` is not yet in the file's icon library; cells use `Icon / CheckCircle2` as a stand-in (visually close — round shape with internal check; same MyID-verified semantics). Recovery path documented in chip.md: when `BadgeCheck` lands, swap the instance reference in the 3 Tier2 cells.
  - `Chip / Count` — component set `78:44` at `(1300, 3720)`, 320×624. 18 cells = 3 digits × 3 tones × 2 borders. `Digits=One` renders as a 20×20 square (centered, no padding-X) — `radius/pill` rounds to a circle. `Two` and `Overflow` (renders `99+`) auto-hug width with `space/2` (8pt) horizontal padding. `Border=Stroked` adds a 2pt `color/background` outside ring — invisible against page background by design (it "punches" the badge out of the host icon when overlaid on a tab-bar bell / mailbox; ring blends into the page surface, becomes visible against the icon). Label weight is **layer-level Semibold override** on the `text/label` Style — spec calls for `font-semibold` (600) but `text/label` is Medium (500); deferred a `text/label-semibold` Style addition until a 2nd 13pt-Semibold consumer surfaces.
- **Foundation refinement surfaced**: the `text/label` Text Style ships with `textCase: UPPER` + `letterSpacing: +0.04em` baked in (per `figma-setup.md` "Chips, kbd, **uppercase** section labels" use). Chip labels are firmly Title Case per LESSON 2026-05-02 + chip.md anti-pattern row. Resolved by overriding `textCase = 'ORIGINAL'` + `letterSpacing = 0%` at the layer level on every chip label cell that uses `text/label` (39 cells across the three sets — every Sm + Md + every Tier label + every Count label). The override is documented in chip.md's "Deviations from spec, tracked" table; if a 4th consumer needs the same override, lift to a new `text/label-titlecase` Style.
- **Frozen status — lock icon deferred**: spec line "Frozen — slate-500 (with lock icon overlay)" not implemented; `Icon / Lock` not yet in the icon library. Recovery path documented in chip.md (add `Icon / Lock`, append small leading icon slot 10/12/14pt to the 3 `Frozen` cells). Admin-only territory (card status); chip remains unambiguous without the glyph.
- **Variable bindings — every chip cell binds to**: container fill (5 surface palettes) · padding-X (`space/2`–`space/3`) · item-spacing (`space/1`) · corner radius (`radius/pill` on all 4 corners) · dot fill (semantic 600-stop) · label fill (semantic 700-stop or `color/primary-foreground`). No raw hex anywhere in the chip layer. Stroke binding only on `Border=Stroked` count cells.
- **Skip cells**: zero. All 57 variant combinations authored — no `Type x State x Icon` style explosion that would create skip-cells (chips are non-interactive at the primitive layer; state + icon belong to the host row, not the chip).
- **Pass 2 progress**: `2 of 5 primitives` now have their Figma sections (Button + Chip). Remaining: Avatar (3 sizes × {initials, photo} + status-dot — small, ~6–9 cells) · Input (7 variants × ~4 states × density — biggest, ~30+ cells) · Icon (likely a frame-index of the canonical lucide set, not a true variant set — confirm shape with user before authoring).
- **Lessons applied**: 2026-04-29 typography 13pt floor (chip labels `text/label` 13pt min) · 2026-05-01 buttons + flowing text never below 14pt (N/A — chips are reserved for `text-xs`) · 2026-05-02 Title Case + no uppercase tracking-wider on chip labels (drove the 39-cell `textCase: ORIGINAL` override).
- **Verification**: Two `await screenshot()` round-trips confirmed visual fidelity per set: StatusChip after the Title Case fix, TierBadge after the glyph + override, CountBadge after the 18-cell sweep. Each set rendered cleanly with proper colors, dot/glyph sizing, and pill geometry.
- **Open items**: 4 deviation rows tracked in chip.md "Deviations from spec, tracked" table — none blocking, all with documented recovery paths. Pass 2 continues with Avatar next per user-approved sequence (Chip → Avatar → Input → Icon).
...
- **Token wiring path locked**: native **Figma Variables (Color w/ Light + Dark modes · Spacing · Radius · Duration) + Styles (Text + Effect)**. Tokens Studio plugin demoted to optional sync path. Single canonical frame per component; mode swaps via Variables (not 3 frame variants per state). Variant-axis explicitness added to specs so Figma component-set assembly is mechanical, not interpretive. User checks Figma manually — no review handoff loop on my side.
- **Pass 1 — Token Figma-setup (DONE)**: new [`mobile/design/tokens/figma-setup.md`](../mobile/design/tokens/figma-setup.md) (~330 lines) — canonical Variables collections (Color w/ 2 modes covering 11 brand stops + 11 slate stops + 3 success/warning/danger ramps + 19 semantic role aliases · Spacing 13 stops · Radius 4 stops · Duration 3 stops); 9 Text Styles (6 sans + 3 mono w/ tabular-nums + Inter font features cv11/ss01/ss03); 8 Effect Styles (4 elevations × 2 modes — `effect/shadow-hero/light` + `/dark` brand-tinted; Effect Styles aren't mode-aware in Figma today, so 8 separate styles); slash-separated naming (`color/brand/600`, `space/4`, `radius/lg`, `text/body`, `effect/shadow-md/light`); component-set naming `Component / Variant / Size / State / Modifier`; 9-step setup-ordering; Tokens Studio sync option for users who'd rather import; lessons-binding-this-layer summary (type floor, tap-target, color-only-signals, sticky-chrome rules, padding contract, Visa/MC scope, privacy boundary). [`tokens/README.md`](../mobile/design/tokens/README.md) updated to point at `figma-setup.md` as canonical setup path; Tokens Studio + Tailwind paths demoted to "optional / downstream code consumers."
- **Pass 2 — Primitive sweep (PARTIAL — 1 of 16 files)**: `primitives/button.md` swept w/ new "Figma component-set" section between Anatomy and Token consumption. 4-axis variant matrix (Variant 5 × Size 3 × State 5 × Icon 3 = 225 cells; 195 effective after skip-cells excluding Loading×Trailing-icon + Link×Lg combos). Per-cell Variable bindings for fill / label color / corner radius / padding / focus-ring Effect Style. Auto Layout values bound to `space/*` Number variables. Text Style binding per size (Body small Medium for Sm/Md, Body Semibold for Lg). Tertiary + Link bind to primitive ramp (`color/brand/700` light · `/400` dark) directly with manual mode swap noted — flagged for potential future `color/semantic/foreground-brand` alias if it becomes painful across primitives.
- **PAUSED at button.md**: user asked whether I could access the Figma file directly via URL after sharing the link. **Verified via ToolSearch + ListMcpResourcesTool**: NO Figma MCP server is wired in current Claude Code config (only `claude.ai Notion` / `Gmail` / `Google Calendar` / `Google Drive` MCP servers available — none Figma). Two install paths offered to the user: **(A)** official Figma Dev Mode MCP at `http://127.0.0.1:3845/mcp` — Figma desktop app required, **read-only** (inspect frames / variables / styles, suggest adjustments, audit naming consistency); **(B)** community `figma-developer-mcp` via npx — Figma personal access token required, **read + write** (build Variables / Styles / component sets / patterns directly in the file via Figma Plugin API). Recommended **B** for the volume ahead (5 primitives + 11 components + 7 patterns + 11 surfaces × multiple variants each = hundreds of cells; manual implementation = weeks; MCP-driven = days). User installs MCP server → restarts session → Pass 2 resumes with full Figma authoring rights against the prepped specs. Remaining Pass 2 scope: 4 primitives (input/chip/avatar/icon) + 11 components + 2 READMEs.
- **Doc cascade**: [`docs/product_states.md`](../docs/product_states.md) — last-updated bumped, mobile-app surface paragraph extended w/ Figma-as-implementation-surface note, Tokens row file count bumped 9→10 + figma-setup.md described, Mobile primitives row notes button.md Figma section + lock criteria. [`ai_context/AI_CONTEXT.md`](./AI_CONTEXT.md) — new Phase 25 entry in Active workstreams covering Pass 1 done + Pass 2 partial + MCP install path. [`ai_context/HISTORY.md`](./HISTORY.md) — this entry.
- **No PRD / models / mermaid cascade** — Figma setup is a tooling change for the design layer, no schema / state-machine / product spec changes.
- **No new LESSONS** — the binding lessons (type floor, sticky chrome, padding contract, Visa/MC scope, privacy) are existing and already documented; the Figma decision doesn't introduce a new pitfall, only a new tooling surface.

**Files touched**:
- new — `mobile/design/tokens/figma-setup.md`
- modified — `mobile/design/tokens/README.md`
- modified — `mobile/design/primitives/button.md`
- modified — `docs/product_states.md`
- modified — `ai_context/AI_CONTEXT.md`
- modified — `ai_context/HISTORY.md`

---

### 2026-05-04 — Mobile Primitives + Components layers DRAFTED (in review) — 18 spec files (~3,593 lines) in `mobile/design/primitives/` and `mobile/design/components/`; layers 2 + 3 of 6 in the mobile design system

- **Summary**: Two-layer cascade after Tokens locked. Per the layer hierarchy (Tokens → Primitives → Components → Patterns → Screens → Flows), drafted the next two layers in one continuous pass — both internally complete, both still in review (status `🚧`, not `✅`). Every spec mirrors the rhythm established by the Tokens layer (status header → source of truth → variants/sizes → states → anatomy → token consumption → composition rules → accessibility → localization → forbidden patterns → quick grep → cross-references) and every spec cites the canonical lessons that bind it. No actual React code shipped — this is the design-spec layer that drives the prop signatures and visual contract upstream code will consume.

  **Primitives layer** (`mobile/design/primitives/`, 6 files, ~1,351 lines):
  - [`button.md`](../mobile/design/primitives/button.md) (205 lines) — 5 variants (`primary` / `secondary` / `tertiary` / `destructive` / `link`) × 3 sizes (40 / 48 / 56pt — `lg` lifts to `text-base font-semibold`) × 6 states (idle / hover / pressed / disabled / focused / loading). Loading width-freeze contract. Disabled-because-X reason text rendered inline (never tooltip-only on mobile) sourced from `error_codes`. `tertiary` splits admin's `ghost` to give text-only buttons their own intent. Mirrors admin's [`<Button>`](../dashboard/src/components/ui/button.tsx) cva shape.
  - [`input.md`](../mobile/design/primitives/input.md) (304 lines) — 7 variants: text / textarea / **phone** with `+998 ` prefix locked (UZ-only v1, mask `90 123 45 67`, `inputmode="tel"`) / **OTP** 4 or 6 boxes with auto-advance + paste distribution + `autocomplete="one-time-code"` / **PIN dot** (scaffolded for v1.1) / **numberPad** (custom amount-entry keypad, 72×72pt cells, locale-aware decimal separator) / search (lucide leading magnifier + clear-X, 250ms debounce). Value type `text-base` (16pt) to dodge iOS auto-zoom-on-focus. Radius `radius-sm` 8pt (mobile floor, vs admin's 4pt). Privacy boundary: no free-text PAN/CVV inputs in this layer — those go through hosted forms / 3DS per [`card-schemes.md`](../.claude/rules/card-schemes.md).
  - [`chip.md`](../mobile/design/primitives/chip.md) (240 lines) — StatusChip mapped strictly to canonical transfer / KYC / card states (never invent — propose state-machine update first per [`status-machines.md`](../.claude/rules/status-machines.md)) · TierBadge with **mobile-only MyID `<BadgeCheck>` glyph on `tier_2`** (the only mobile-specific deviation in this primitive) · CountBadge with 20pt diameter floor (so type stays at 13pt, doesn't drop below the floor). 28pt default height, `radius-pill`, `text-xs font-medium` (chip is one of the reserved `text-xs` categories per LESSON 2026-04-29). Title Case labels — never `uppercase tracking-wider` per LESSON 2026-05-02.
  - [`avatar.md`](../mobile/design/primitives/avatar.md) (188 lines) — 3 sizes (32 / 48 / 72pt), initials + photo variants, **deterministic name-hash color derivation** (MD5-bucket → brand / slate / success / warning surfaces; never danger-600, never random). Loading state falls through to initials, never a broken-image icon. Status-dot affordance scaffolded but mostly unused at v1 (support-agent context only). Privacy: never auto-source MyID document scan as user avatar.
  - [`icon.md`](../mobile/design/primitives/icon.md) (254 lines) — lucide-react ONLY (no inline SVG paths, no Unicode arrows, no other libraries). 20pt default (`size-5`) — bumped from admin's 16pt for touch ergonomics. Icons inherit `currentColor` from parent (no hardcoded color props). Decorative (`aria-hidden="true"`) vs meaningful (`aria-label="{intent}"`) contract spelled out. The 8 canonical demo icons fixed: Send / ArrowDownToLine / CreditCard / User / ShieldCheck / Clock / ArrowRight / Check. Brand scheme logos live in a separate `<SchemeLogo>` primitive (Components layer), not this one.
  - [`README.md`](../mobile/design/primitives/README.md) (160 lines) — Layer index + cross-cutting decisions table (tap target ≥ 44pt, type floor, focus ring contract, lucide-only, localization, privacy) + admin parity table (deviations per primitive) + sample-app-shell deferred to Components layer + 6 lessons-binding-this-layer + how-to-consume preview snippets + adding-to-this-layer steps.

  **Components layer** (`mobile/design/components/`, 12 files, ~2,242 lines):
  - [`card.md`](../mobile/design/components/card.md) (205 lines) — 6 variants: resting / header-body / footer-action / flat (1pt outline, no shadow) / interactive (whole-card-as-tap with `active:scale-[0.98]`) / **`card-as-object`** (home hero, brand gradient `brand-700 → brand-900` light / `brand-600 → brand-950` dark, `radius-lg` 20pt, `shadow-hero`, 9:5.5 aspect ratio, mimics ISO/IEC 7810 ID-1 credit card). **Strict padding contract per LESSON 2026-05-03**: never half-override `<CardHeader pb-0>` / `<CardContent p-0>` — take the contract whole, or replace with raw elements that own their own padding. Privacy: card-as-object PAN is masked-only.
  - [`list-row.md`](../mobile/design/components/list-row.md) (228 lines) — 5 variants: single-line (56pt) / two-line (64pt) / avatar (72pt with 48pt avatar) / toggle (whole-row toggles the switch) / selectable (radio left, group container `role="radiogroup"`). Touch-first heights (taller than admin's dense `<TableRow>`). Whole-row tap-target ownership — never both a row tap AND a nested button. Composition rule for cards-with-list-rows: card owns side padding via `overflow-hidden`, rows zero out their `px`.
  - [`banner.md`](../mobile/design/components/banner.md) (169 lines) — 4 tones (info / success / warning / danger). **Inline only, NEVER sticky** (per LESSON 2026-05-02 + 2026-05-03 chrome-stickiness rules). 10 canonical use cases tabled (tier-upgrade / MyID-expiring / MyID-expired / FX-stale / provider-down / sanctions-review / transfer-success / offline / card-frozen). Error / compliance copy sourced from `error_codes`. Multi-signal (color + icon + label trio). CTAs are `tertiary` or `link` only — never `primary` (banners are context surfaces, not call-to-action).
  - [`toast.md`](../mobile/design/components/toast.md) (185 lines) — 4 tones, top-of-screen on mobile (admin uses bottom-right). 4s auto-dismiss for success/info, 6s for warning, **manual dismiss for error** (error toasts never auto-dismiss). Swipe-right gesture + keyboard fallback. Max 3 stacked, oldest dismisses early. **Reserved for non-blocking errors only** — transfer failures, compliance reviews, KYC errors all go to banners or modals, never toasts (per [`error-ux.md`](../.claude/rules/error-ux.md)).
  - [`segmented-control.md`](../mobile/design/components/segmented-control.md) (162 lines) — `inline-flex` default + **`fullWidth` mode for fixed-bottom mobile action bars** (per LESSON 2026-05-03 — every row in the action bar matches edge-to-edge width). Pill-shaped track + thumb. 2 or 3 segments (4+ → Tabs component). Used for filtering / view-switching, NEVER as a navigation primitive.
  - [`tab-bar.md`](../mobile/design/components/tab-bar.md) (175 lines) — 4 tabs (Home / Send / History / More — locked v1 commitment). **NO FAB, NO diagonal cutout, NO raised pill** — slight `scale-[1.06]` on the Send center tab is the only emphasis allowed. 56pt content + safe-area-bottom buffer. CountBadge overlay (`top: -4pt right: -4pt`). Multi-signal active state (brand color + active dot above icon + slight scale + label weight). Routes that hide the tab bar enumerated (onboarding, sign-in, MyID, send/review, send/processing, send/result, modals).
  - [`sheet.md`](../mobile/design/components/sheet.md) (185 lines) — 3 snap points: `peek` (25%) / `half` (50% default) / `full` (92% — preserves 8% page-peek above; for full coverage use Modal). Drag handle 36×4pt always visible (primary affordance). `radius-lg` top corners only, bottom flush to safe-area. 9 canonical use cases tabled. MyID step sheets are `dismissible: false`.
  - [`modal.md`](../mobile/design/components/modal.md) (189 lines) — **Full-screen on mobile** (no centered-card-on-dim — that's a desktop pattern). 3 variants: `confirm` / `destructive` (initial focus on Cancel by default — prevents accidental confirms) / `info` (single OK). CTA stack vertical, primary on top, secondary below, both `w-full`. 8 canonical use cases tabled. Reserved for confirmation; multi-step confirm forbidden — use a screen flow.
  - [`headline-number.md`](../mobile/design/components/headline-number.md) (192 lines) — The FX hero. 3 sizes: `display` 44pt (home card-as-object balance, receipt amount) / `hero` 32pt (send-money review pair, transfer-detail summary) / `inline` 16pt (list-row trailing meta). `tabular-nums` always. **Locale-aware separators driven by `users.preferred_language`** (not device locale): space + comma for `uz`/`ru`, comma + period for `en`. **Bigint minor-unit input only — never floats**. NBSP between amount and currency code (never wraps). Decimal-display rules per surface (UZS ≥100 = 0 decimals except review/receipt; CNY always 2). 4 modifiers (signed / muted / strikethrough / placeholder).
  - [`status-timeline.md`](../mobile/design/components/status-timeline.md) (167 lines) — Vertical event timeline tied strictly to canonical state machines (transfer / KYC / card). Past = filled circle + solid line; current = ring + inner pulsing dot + dashed forward line; future = hollow + dashed. 12pt circles, 32pt line spacing. State-tone mapping per [`status-machines.md`](../.claude/rules/status-machines.md). Event row data shape (state / actor / created_at / failure_code / context) tied to `transfer_events`. `failure_code` localized via `error_codes`. Customer surface NEVER offers state-control CTAs (admin-dashboard-only).
  - [`scheme-logo.md`](../mobile/design/components/scheme-logo.md) (174 lines) — Brand-accurate marks for UzCard / Humo / Visa / Mastercard / Alipay / WeChat. 3 sizes (32×20 / 56×32 / 88×56pt — width sets size, height follows ~1.75:1 aspect ratio). lucide-react `<CreditCard>` is forbidden as a fallback — render skeleton, not generic icon. **Visa / Mastercard registered in the primitive but excluded from v1 mock data per LESSON 2026-04-30** — opens up only when user explicitly invokes them. Disabled state uses `opacity-50` + helper text (never tinted brand asset). Always paired with masked PAN (first6 + last4) per [`card-schemes.md`](../.claude/rules/card-schemes.md).
  - [`README.md`](../mobile/design/components/README.md) (211 lines) — Layer index + cross-cutting decisions table (padding contract · sticky-vs-inline chrome · type floor · color-only signals forbidden · state-machine canonicality · money correctness · Visa/MC scope · privacy boundary) + admin parity table per component + 12 lessons-binding-this-layer + how-to-consume preview snippets + sample-app-shell deferred-as-render-artefact note + Patterns-layer next-pass pointer.

  **Cross-cutting decisions binding both layers** (every spec cites and honors):
  - **Tap target** ≥ 44 × 44pt every interactive element (visual can be smaller, tap-area inflates via padding/wrapper)
  - **Type floor** — buttons + flowing text ≥ `text-sm` (14pt) per LESSON 2026-05-01 · input value text ≥ `text-base` (16pt) — prevents iOS auto-zoom on focus · `text-xs` (13pt) RESERVED for chip/kbd/uppercase tracking-wider section labels/avatar initials/tooltip body/tab-bar labels per LESSON 2026-04-29
  - **Color-only signals forbidden** — every status / variant pairs color with icon + label trio · selected states pair tint + ring + indicator · active tab gets brand color + active dot + slight scale + label weight
  - **State machine canonicality** — chip / banner / timeline / status displays pull from `transfer_state_machine.md` / `kyc_state_machine.md` / `card_state_machine.md`. Never invent states (LESSON 2026-05-03 narrowing). User-controllable state-transition CTAs in customer surface are forbidden
  - **Money correctness** — Headline-Number accepts bigint minor units, never floats. UI divides by 100 at display. Locale-aware separators driven by `users.preferred_language`
  ...
  - **Privacy boundary** — no primitive accepts free-text PAN / CVV / full PINFL / full document number. Card flows go through hosted forms / 3DS WebView. SchemeLogo always paired with masked PAN. Card-as-object PAN is masked-only
  - **Sticky-vs-inline chrome** — banners inline only, never sticky · tab strip on tabbed surfaces NEVER sticky (LESSON 2026-05-03) · detail-page bottom action bar uses canonical `fixed inset-x-0 bottom-0` overlay (LESSON 2026-05-02) · tab bar `bottom: 0` always present in app shell

  **Cross-surface parity to admin**: Every primitive and component links to its admin counterpart in `dashboard/src/components/` and enumerates mobile-specific deviations explicitly. Brand + slate + semantic palettes inherit verbatim. Mobile-specific additions: TierBadge MyID glyph on `tier_2`, Card's `card-as-object` variant, Headline-Number's `display`/`hero` sizes (consuming Display 1 / Display 2 typography roles only mobile has), SegmentedControl's `fullWidth` mode, full-screen Modal layout, Sheet (no admin equivalent).

  **No PRD / models / mermaid cascade** — design-system layers, no schema changes, no state machines touched, no flow diagrams updated.

  **Doc cascade scope** — `docs/product_states.md` (mobile primitives row ❌ → 🚧 split into `Mobile primitives` + `Mobile components` + `Mobile patterns`; mobile section opener updated; "Last updated" stamp bumped) + `ai_context/AI_CONTEXT.md` (Active workstreams: mobile primitives + components ☐ → 🚧 with full bills of materials) + this HISTORY entry.

  **What's next**: Either the user **locks** the Primitives + Components specs (status flips 🚧 → ✅), or they redirect specific specs for revision. Once locked, the next pass is **Patterns** — KYC step / FX-quote breakdown / recipient picker / send-money review block / receipt block / transfer-status detail / card-link 3DS WebView wrapper. The sample app shell render artefact (light / dark / 200% dynamic-type) lands once Components are locked.

- **Files**:
  - `mobile/design/primitives/button.md` (NEW)
  - `mobile/design/primitives/input.md` (NEW)
  - `mobile/design/primitives/chip.md` (NEW)
  - `mobile/design/primitives/avatar.md` (NEW)
  - `mobile/design/primitives/icon.md` (NEW)
  - `mobile/design/primitives/README.md` (NEW)
  - `mobile/design/components/card.md` (NEW)
  - `mobile/design/components/list-row.md` (NEW)
  - `mobile/design/components/banner.md` (NEW)
  - `mobile/design/components/toast.md` (NEW)
  - `mobile/design/components/segmented-control.md` (NEW)
  - `mobile/design/components/tab-bar.md` (NEW)
  - `mobile/design/components/sheet.md` (NEW)
  - `mobile/design/components/modal.md` (NEW)
  - `mobile/design/components/headline-number.md` (NEW)
  - `mobile/design/components/status-timeline.md` (NEW)
  - `mobile/design/components/scheme-logo.md` (NEW)
  - `mobile/design/components/README.md` (NEW)
  - `docs/product_states.md` (mobile primitives row split into 3 rows: primitives 🚧 + components 🚧 + patterns ❌; mobile-app section opener updated; "Last updated" stamp bumped)
  - `ai_context/AI_CONTEXT.md` (Active workstreams: mobile primitives + components ☐ → 🚧 with full per-file bills of materials)
  - `ai_context/HISTORY.md` (this entry)
